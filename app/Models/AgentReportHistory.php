...
/**
 * Historique de DÉBOGAGE des rapports agent : payload brut
 * complet, append-only, écrit uniquement quand le flag
 * `config('agent.report_history')` (`AGENT_REPORT_HISTORY`, défaut off)
 * est actif.
 *
 * Table temporaire par design (retrait prévu à la sortie de débogage) —
 * supprimable d'un bloc sans toucher l'état courant ni le
...
{
    /** Nom singulier voulu — pas le pluriel Eloquent `histories`. */

...
/**
 * Modèle de RÉCEPTION du contrat amont émis par l'autorité controlHub.
 *
 ...
 *
 * Persistance uniquement : structure et relations. L'ingestion d'un payload reçu
 * et le branchement dans `StateCompiler` (tier de précédence amont > local)
 * vivent ailleurs.
 * - Mapping label → WorkstationGroup local →.
 * - Bornage catalogue & install →.
 *
 * ⚠️ Convention de nommage : aucun mot « central » dans ce modèle, ses colonnes, ses relations.
 ...
 * @property ?string $schema_version Version du schéma d'échange du dernier payload reçu
...
class ControlHubContract extends Model
{
    ...
    /**
     * Résout le contrat amont **actif** (singleton « ≤ 1 contrat
     ...
     *  : filtre sur `link_state = active`. Ne pas diverger (heuristique
     * « dernier reçu » interdite). Sans contrat actif → null, aucun label

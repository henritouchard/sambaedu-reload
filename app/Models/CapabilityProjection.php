...
/**
 * Projection d'une {@see Capability} : comment l'intention se
 ...
 * Le `mechanism` est un identifiant FIGÉ aligné sur
...
class CapabilityProjection extends Model
{
    ...
    /**
     * Mécanisme liste registre à sous-valeurs indexées `\1..\N` (
     ...
     * numérique de la clé-conteneur. Une capacité peut
     * porter registry ET registry_list sur le même OS (bi-projection —
     * l'unique `(capability_id, os, mechanism)` le permet).
     */
    public const MECHANISM_REGISTRY_LIST = 'registry_list';

    /**
     * Mécanisme HORS-REGISTRE `fs_acl` (contrat §7.7 — type
     ...
     * dernier-appliqué), jamais la DACL entière (chirurgie
     * SetNamedSecurityInfo). Le serveur n'émet que des NOMS (`Eleves`,
     * `Domain Users`) ; la résolution SID est côté POSTE (LSA) — provider
     * Postgres pur.
     */
    public const MECHANISM_FS_ACL = 'fs_acl';

    /**
     * Mécanisme HORS-REGISTRE `firewall` (contrat §7.8 — type
     ...
     * marqueur de propriété — PAS de store, contrairement à `fs_acl`) : il le
     ...
     * inverses-RFC1918 vit dans le HANDLER. La politique par défaut, les
     * profils et le service MpsSvc ne sont JAMAIS touchés — provider Postgres pur.
     */
    public const MECHANISM_FIREWALL = 'firewall';

    /**
     * Mécanisme HORS-REGISTRE `privilege` (contrat §7.9 — type
     ...
     * POSTE (LSA) — provider Postgres pur.
     */
    public const MECHANISM_PRIVILEGE = 'privilege';

    /**
     * Mécanisme `legacy_cleanup` (contrat §7.10 — type
     ...
     * enum FERMÉ (`vanilla` seule valeur : suppression des paires
     * profiles.ini/installs.ini référençant `sambaedu.default`, AUCUN profil
     * forcé posé). Le CATALOGUE d'artefacts (blobs applications-*,
     * tâches WPKG, scripts GPO locale, helpers, autologon se4install) est
     * versionné DANS l'agent (connaissance legacy figée, pas du
    ...
    /**
     * Mécanisme HORS-REGISTRE `app_profile` (contrat §7.11 — type
     ...
     * radical `sambaedu` : jamais matché par la garde
     * `referencesSambaeduProfile` du mécanisme `legacy_cleanup`, les
    ...
    /**
     * Hint « geste de rafraîchissement » — shell_notify (contrat §7.1/§7.6).
     * Convention d'authoring : champ OPTIONNEL `refresh` au NIVEAU
     ...
     * (`ParseRefreshLevel`), le catalogue n'écrit jamais que la forme
    ...
    /**
     * Marqueur d'exécutant SYSTEM — champ `writer` (contrat
     ...
     *
     * **Exclusion mutuelle `refresh`/`writer`** :
     ...
     * capability-first (le modèle `RegistrySetting` est superseded —).
     */
    ...
    /**
     * Ruche des profils utilisateurs HKEY_USERS — TROISIÈME valeur

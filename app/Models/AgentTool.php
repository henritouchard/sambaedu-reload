...
/**
 * Outil de rendu du catalogue agent.
 *
 ...
 * AVANT extraction — l'agent ne porte plus de checksum figé en dur.
 *
 ...
 * `save`/`updateOrCreate` directement. SECOND écrivain, en CRÉATION SEULE :
 * {@see \App\Services\ControlHub\ArtifactPullService} peut
 * `updateOrCreate(['key' => ...])` un outil tiré du canal ④ amont, mais
 * UNIQUEMENT quand la clé est absente localement (précédence : le pull ne
 ...
 *
 * `enabled` : toggle GLOBAL — au plus un outil par `key` (mono-version).
 * Désactivé → no-op côté agent, jamais de désinstallation.
 */

...
/**
 * Une extension du registre local SE5.
 *
 ...
 * le registre d'extensions est isolé PAR CONSTRUCTION.
 *
 * @property int $id
 ...
 * @property string $health_status '' (jamais sondé) | 'ok' | 'unreachable'
...
class Extension extends Model
{
     ...
     * pouvoir le muter par assignation de masse. Décision (tranchée) :
     ...
     *
     * ⚠️ : `installed_version` / `installed_sha256` /
     ...
     *
     * ⚠️ : `health_status` / `health_checked_at` /
    ...
    /**
     * Bloc `install` du manifest v1 (extension ADDITIVE) : ce qui
     * permet d'installer une `app`. Absent ⇒ `null` (un manifest sans
     * bloc `install` reste valide).
     *
     ...
     * index VÉRIFIÉ contre la clé pinnée de la source : la vérification
     * du paquet contre cette valeur EST donc la vérification « contre la clé
     * déclarée de sa source », à la manière d'apt
    ...
    /**
     * Le manifest porte-t-il un bloc `install` EXPLOITABLE par le
    ...
    /**
     * Cette extension a-t-elle un backend à SONDER ?
     *
     ...
     * sans port », — une ligne fabriquée à la main n'a rien derrière elle).
    ...
    /**
     * L'état de santé PERSISTÉ est-il périmé (ou inexistant) ?
     *
    ...
    /**
     * **LA règle d'affichage « Indisponible »**, en UN SEUL
     * énoncé (patron {@see ExtensionSource::offersAvailableExtensions()}) : une
     * garantie qui n'existe que dans la vue n'est pas une garantie, et une règle
     * n'a qu'un seul énoncé.
     *
     ...
     *
     * ⚠️ Un badge n'est JAMAIS une autorisation : la tuile marquée reste
    ...
    /**
     * ÉCART BRUT entre la version qui TOURNE et celle que le
     * catalogue PUBLIE. C'est un **fait**, pas une décision.
     *
     * ⚠️ Ne pas confondre avec
     * `ExtensionCatalogService::hasUpdateAvailable`, qui est la
     ...
     * bouton.
     *
     * `hasUpdateAvailable()` **compose** cette méthode au lieu de recopier son
     * expression : le fait n'a qu'un énoncé, la décision l'utilise.
     */
    ...
    /**
     * Scopes DEMANDÉS par le manifest — information admin : affichés, jamais
     * consommés. Les scopes EFFECTIFS sont ceux accordés au client OIDC.
     *
     * @return list<string>
     */
     ...
     * Le catalogue STOCKE cette donnée ; c'est le lanceur qui la RÉSOUT.

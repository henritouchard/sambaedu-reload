 ...
 *
 * **19 cases stricts postérieur** (3 historiques + 9 Linux install + 7 Windows
 * install) :
 *
 *  - 3 cases (maintenance) :
 ...
 *  - `InstallUbuntu64` — Ubuntu Focal (hors domaine, perso=1).
 *
 *  - 7 cases (installation Windows) :
 ...
 *
 * **HORS-SCOPE (déférés Phase 3)** : `se4ad`, `se4fs`, `deb_serv`,
 * `deb_kiosk`, `deb_nextcloud`, `deb_gnome_perso`, `primtux`.
 *
 * **HORS-SCOPE (déférés)** : `installw11old` (variante Win11-old),
...
enum IpxeAdminAction: string
{
    ...
    /**
     * Retourne la valeur utilisée pour `MachineBootLog.action` lors de la
     * persistance d'une action.
     *
     * **Mapping** :
     *
     *  - Actions Clonezilla (live/save/restore) → `'ipxe_clonezilla'` (15 chars).
     *  - `gparted` → `'ipxe_gparted'` (12 chars).
     *  - `hdt` → `'ipxe_hdt'` (8 chars).
     *  - `memtest86plus` → `'ipxe_memtest'` (12 chars).
     *  - 9 cases install_* → `'ipxe_<distro>_<variant>'`
     *    (audit fin — ex: `'ipxe_deb_gnome'`, `'ipxe_nird'`, `'ipxe_ubuntu64'`).
     *  - 7 cases install_win* → `'ipxe_<version>_<flag>'`
     *    (audit fin — ex: `'ipxe_win10'`, `'ipxe_win11_perso'`).
     *  - Cases héritage (`rescuecd`, `winpe`, `factory_reset`) → `'ipxe_action'`
     *    (compat historique — voir « Divergence intentionnelle » ci-dessous).
     *
     * **Divergence intentionnelle — FactoryReset vs ClonezillaRestoreSda2Sda1** :
     *
     ...
     * remonter quel chemin UX a été emprunté (menu factory_reset vs
     * menu clonezilla restore).
     *
    ...
    {
        return match ($this) {
            // 6 cases clonezilla/diagnostic (audit fin).
            self::ClonezillaLive,
            ...
            // Cases (Rescuecd, Winpe, FactoryReset) — compat historique.
            // Voir PHPDoc « Divergence intentionnelle » ci-dessus.
            default => 'ipxe_action',
        };
    }

    /**
     * Retourne les metadata Linux d'un case install_*.
     *
    ...
    /**
     * Retourne les metadata Windows d'un case install_win*.
     *

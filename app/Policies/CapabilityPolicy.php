...
/**
 * Policy d'écriture d'une capacité, gate `modify-capability`.
 *
 ...
 * patron d'ENREGISTREMENT (`RegistersGates` + propriété `$gates`), PAS
 * sa logique de délégation.
 *
 * ⚠️ Depuis ce gate ne reflète QUE le verrou amont (plus aucun droit) : un
 ...
 * `PermissionService::canOnWorkstationGroup`. Sa PORTÉE dépend du canal :
 ...
 *
 * **Distinct du gel local `overrides_locked`** : ce dernier gèle l'AJOUT
 ...
 * coexistants.
 *
 * **ce gate ne porte PLUS de plancher de droit.** `modify-capability`
 * est DUAL-PURPOSE : il sert l'override PAR-PARC (`capabilities-tab`, droit SCOPÉ
 * `customize-workstationGroup`) ET le défaut diffusé GLOBAL d'instance
 ...
 * ne conserve QUE le verrou amont. Avant il imposait en plus un plancher de
 * droit GLOBAL `app.customize` qui rebloquait le délégué positif-seul que le guard
 * scopé venait d'autoriser — plancher retiré comme redondant et
 * nuisible. Sans contrat amont actif, le resolver court-circuite : la
...
class CapabilityPolicy
{
     ...
     *
     * `$user` est INUTILISÉ : le contrôle de droit est porté par

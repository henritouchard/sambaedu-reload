...
/**
 * Résolution du manifest tool/skin servi à l'agent.
 *
 * Endpoint manifest DÉDIÉ (iso `release-manifest`) — délibérément HORS
 ...
 *    sha256, size}` — l'agent y lit le SHA-256 attendu du portable, à la
 *    place de la constante Go `RainmeterToolChecksum`. Outil absent ou
 *    désactivé → `tool: null` → no-op gracieux côté agent ;
 ...
 *
 * Lecture seule (`agent_tools` + fichier skin) ; aucune écriture, et zéro
...
class AgentToolManifestService
{
    ...
    private function activeTool(): ?array
    {
        ...
        if ($tool === null) {
            return null; // absent ou désactivé → no-op gracieux côté agent
        }

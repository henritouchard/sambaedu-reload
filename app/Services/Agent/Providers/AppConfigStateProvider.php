...
/**
 * Type `app_config` (contrat §7, identifiant figé,
 * {@see AppCustomization::TYPE_APP_CONFIG}) — projection en LECTURE SEULE des
 * policies d'app résolues (`app_customizations`) vers des candidats d'état.
 *
 * **Aucune table dédiée à ce provider.** La table métier `app_customizations`
 * et son service de résolution `AppCustomizationService` existent déjà : on les
 * LIT, on ne les double pas — une 2ᵉ table de policies serait un doublon de
 * source de vérité.
 *
 * **Lecture Postgres + config PURE** (critère Keycloak).
 ...
 *
 * **Sémantique `aggregate` PAR `app_kind`.** Un poste reçoit la
 ...
 * hiérarchique a DÉJÀ fusionné les 6 niveaux en UN `policies_json`. Le
 ...
 * `resolvePoliciesForMachine`), comme `WallpaperResolver` — elle ne relève pas
 * de la précédence de maille, qui reste au compilateur.
 *
 * **Scope `machine`.** Le legacy traite Firefox via DEUX mécanismes distincts
 * qui coexistent :
 ...
 *     **C'est le périmètre de ce provider.**
 *   - **Mécanisme B — profil user** = jonctions/redirection du dossier profil
 *     vers le home (roaming). **Hors périmètre ici** : relève du roaming.
 ...
 * `registry` HKLM). Le par-user de Firefox = le PROFIL (Mécanisme B), PAS
 ...
 * + WG) avec `$user = null`.
 *
 * **Impédance « WG unique » vs « mailles ».**
 ...
 *
 * **Payload v1** : `{app_kind, policies}` — `app_kind` =
 ...
 * ni un scope (invariant central). Le payload est NORMALISÉ sans float
...
final class AppConfigStateProvider implements StateProvider
{
     ...
     * déterminisme de `sourceId` pour l'aggregate.
     *
    ...
    {
        // MACHINE : `policies.json` est machine-wide sous Program Files
        // (admin-write) → écrit par le service SYSTEM, jamais par le compagnon
        // user. Résolution PAR PARC (niveaux 1-4) ; le par-user Firefox est le
        // profil (Mécanisme B / roaming), hors périmètre.
     ...
     * MACHINE / par-parc : le par-user Firefox est le profil (Mécanisme B /
     * roaming), hors périmètre. L'agent étant Windows-cible,
    ...
    public function itemsFor(TargetContext $ctx): Collection
    {
        ...
        foreach (AppKind::cases() as $kind) {
            ...
            // (Mécanisme B / roaming, hors), pas par `policies.json`.
    ...
    /**
     * WG représentatif du poste pour la résolution de l'impédance :
     * le parc LOGIQUE l'emporte sur la salle PHYSIQUE (inversion globale,
     * alignée sur `StateCompiler::specificity()`). À défaut de parc, la
     * salle ; à défaut de tout WG, `null` (la chaîne saute simplement le
     ...
     * ne résout PAS les niveaux user 5-6). Sert le tiebreak de
     ...
     * cases), pré-calculé UNE fois. L'ordre aggregate final est trié
     ...
     * float est converti en string (un timeout décimal, une version « », …).

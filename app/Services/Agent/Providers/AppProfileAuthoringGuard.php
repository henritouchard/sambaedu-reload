...
/**
 * Garde-fou d'AUTHORING des projections
 ...
 *   3. **radical `sambaedu`** dans `profile_name`/`link`/`server` :
 *      un nom bâti sur `sambaedu` collisionnerait avec la garde
 *  `referencesSambaeduProfile` du mécanisme `legacy_cleanup`, qui
 ...
 * 6. `enabled` présent mais NON booléen STRICT — le champ
...
{
    /**
     * Radical interdit dans les identifiants de profil, insensible à la
     * casse. Le mécanisme `legacy_cleanup` supprime toute paire
    ...
    public function violations(array $projections): array
    {
        ...
        foreach ($projections as $projection) {
            ...
            foreach ($this->apps($projection['spec'] ?? null) as $app) {
                ...
                // (7) `enabled` booléen STRICT si présent. ABSENT
                // = vaut `true` (défaut — comportement préservé, le provider ne

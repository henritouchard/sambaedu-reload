...
/**
 * Provider `firewall` CAPABILITY-FIRST, portée **Machine** (le
 ...
 * doctrine : « mécanisme = code payé une fois, capacité = donnée »). Il
 ...
 * dans la donnée. Il SURCHARGE l'interpréteur `expand()` du provider abstrait
 * sans toucher `StateCompiler`. Il réutilise `resolveKeyValue()` (map/littéral)
 * et `UNMANAGED` hérités ; lecture Postgres pure.
 *
 * **Propriété PAR CONTENEUR.** Contrairement à `fs_acl` (store « dernier
 ...
 * `remote_scope: internet` en plages inverses-RFC1918 vit dans le HANDLER,
 ...
 *
 * **Écart assumé sur `ensure`.** Le cadrage parlait d'un enum « sans verbe
 ...
 * annule un broadcast `off`, et le groupe finit VIDE : « on ⇒ groupe vide » à la
 * lettre).
 *
 * **Pas de ciblage par utilisateur.** `scope() = Machine` ⇒ le service
...
final class FirewallCapabilityProvider extends AbstractCapabilityStateProvider
{
     ...
     * STABILITÉ de la sélection (déterministe, ETag).
     */
     ...
     *     au render) ; défaut `present`, TOUJOURS émis ;
    ...
    protected function expand(CapabilityProjection $projection, string $capabilityValue): array
    {
        ...
        foreach ($rules as $rule) {
            ...
            // `rule_id` émis en MINUSCULE, cohérent avec

 ...
 * besoin de savoir OÙ poser un `.lnk`. Un SECOND consommateur existe
 * désormais — {@see \App\Services\Agent\Providers\ShellFoldersStateProvider}, qui
...
final class DesktopPathResolver
{
     ...
     * `desktop_sweep_paths` pour le BALAYAGE, `path` pour la REDIRECTION).
     */
    ...
    /**
     * Chemin du bureau résolu côté SERVEUR.
     *
     ...
     * Aucune autre branche réseau/local n'existe côté serveur.
     *
     * **UN SEUL facteur : l'environnement du parc.** Le bureau RÉSEAU vit dans
     * le home SMB (`\\<se4fs>\users\<user>\`), et ce partage-là **est toujours
     * là** : il porte deux choses distinctes qu'il ne faut pas confondre.
     *
     ...
     * et d'y écrire. Le bureau ne suit donc
     ...
     * POSE les `.lnk` — et aussi celui vers lequel il REDIRIGE le shell, les
     * deux sont volontairement le MÊME. Les emplacements qu'il BALAIE sont
     * une autre décision, prise par {@see self::sweepPathsFor()}.
     */
    public function pathFor(WorkstationEnvironment $environment): string
    {
        return match ($environment) {
            // Poste partagé : bureau redirigé RÉSEAU, sans aucune condition —
            // le home SMB qui l'héberge est toujours là pour l'agent.
     ...
     * c'est le serveur qui pilote la liste.
     *
     * **Pourquoi le serveur et pas l'agent.**
     ...
     *    anti-orphelins : un poste partagé nettoie les DEUX
     ...
     * abandonné ne serait plus jamais nettoyé. Effet de bord utile : déplacer la
     * POSE ne fait bouger qu'un seul
     * champ du payload.
     *

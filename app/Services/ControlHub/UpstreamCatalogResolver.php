...
/**
 * Résolveur du CATALOGUE applicatif AMONT (source unique, mémoïsée).
 *
 ...
 * `controlhub_contract_catalog_apps`, livrée). Ce service répond à UNE
 * question : « le canal d'install refnum est-il **borné** à ce catalogue, et si
 * oui, quelles apps sont autorisées ? ».
 *
 * **Correspondance de clé (confirmée par le code)** :
 ...
 * local (qui n'a pas de sens cross-instance).
 *
 ...
 *
 * **Catalogue vide = pas de bornage** : un contrat actif SANS
...
final class UpstreamCatalogResolver
{
     ...
     * Reste `false` en standalone comme sur catalogue vide (court-circuit).
     */
     ...
     * catalogue vide) : consultation/install strictement inchangées.
     */
     ...
     *
     * Court-circuit : sans contrat actif, on ne touche JAMAIS la table
     * `controlhub_contract_catalog_apps` (≤ 1 requête `controlhub_contracts`).
     * Catalogue vide ⇒ pas de bornage (`bounded` reste `false`).
     */
    private function ensureResolved(): void
    {
        ...
        // rompu (`severed`) ⇒ null ⇒ court-circuit, zéro requête catalogue.
        ...
        // Catalogue vide ⇒ pas de bornage (ne verrouille pas le refnum hors
        // de toutes ses apps).
        if ($appKeys === []) {
            return;
        }

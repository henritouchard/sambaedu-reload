...
/**
 * Service du CYCLE DE VIE
 * d'une extension : les deux transitions `available ⇄ integrated` du type
 * `link`, et leur trace d'audit.
 *
 * ## — les transitions du type `app` (levée MAÎTRISÉE du filtre)
 *
 * L' étend le lifecycle PAR AJOUT, jamais par modification :
 ...
 * `status` reste
 * VOLONTAIREMENT hors du `$fillable` d'{@see Extension} (décision
 ...
 * catalogue tient deux invariants — « `syncBundled()` n'écrit jamais `status` »
 * et « le prune respecte les extensions intégrées » : y loger les transitions
 * brouillerait la propriété « ce
 ...
 * du socle intacts et donne son point d'extension naturel : le moteur `app`
 * étendra le lifecycle, jamais le catalogue.
 *
 * **Invariant : no-op ⇒ ZÉRO ligne d'audit.** Intégrer une extension déjà
 ...
 * `toastError`, jamais une 500. Un type `app` est refusé explicitement : ces
 * transitions-là passent par {@see ExtensionInstallService}.
 *
 * Les deux méthodes publiques renvoient des **tableaux plats** — rien
...
class ExtensionLifecycleService
{
     ...
     * @param string $sha256 — empreinte du `.deb` réellement posé (gage de rollback)
     * @return array{changed: bool, status: string}
     */
    public function markAppInstalled(int $extensionId, string $version, int $port, ?User $actor = null, string $sha256 = ''): array
    {
        return DB::transaction(function () use ($extensionId, $version, $port, $actor, $sha256): array {
            ...
            // Déjà installée : ni écriture, ni audit. Le moteur écarte
    ...
    public function markAppRemoved(int $extensionId, ?User $actor = null): array
    {
        return DB::transaction(function () use ($extensionId, $actor): array {
            ...
            // Le staging est purgé par le moteur juste avant :
    ...
    /**
     * Acte la MISE À JOUR réussie d'une extension `app` :
    ...
    public function markAppUpdated(int $extensionId, string $version, string $sha256, ?User $actor = null): array
    {
        return DB::transaction(function () use ($extensionId, $version, $sha256, $actor): array {
            ...
            // Rien à changer : ni écriture (pas même `updated_at`), ni
    ...
    private function transition(int $extensionId, User $actor, ExtensionStatus $target, string $action): array
    {
        return DB::transaction(function () use ($extensionId, $actor, $target, $action): array {
            ...
            // État déjà atteint = no-op propre : ni écriture, ni audit.
            if ($current === $target) {
                return ['changed' => false, 'status' => $target->value];
            }

            // FAIL-CLOSED DE L'INTÉGRATION
            ...
            // tierce » restait contournable. La règle est celle du

...
/**
 * Endpoints d'amorçage LAN NON authentifiés. Les deux chemins d'installation de l'agent (GPO-dispatcher figée pour
 ...
 * `agent.token`. `auth.v1.lan-only` (= EnsureLanIp, subnets
 ...
 *  install (cohérent avec la double-porte). 404 `no_release` si aucune
 ...
 *    {@see ReleaseController::download()} : pattern strict →
 ...
 *    n'est pas initialisée côté serveur (config incomplète, pas une erreur
 *    client).
 *
 * Frontière `agent_*` + zéro AD : ces endpoints LISENT
...
class BootstrapController extends Controller
{
     ...
     * l'écrit à son emplacement définitif `agent.exe`).
     *
     * Confinement realpath iso {@see ReleaseController::download()} :
    ...
    {
        // Lookup DB d'abord : seule la STABLE publiée est servie —
        ...
        // Niveau `debug` + action_type distinct du manifest (
        ...
        // check-in (contrat). Distinguer manifest vs binaire évite
     ...
     * côté serveur (`php artisan auth:ca:init` est le prérequis),
     * jamais 500.
     */

...
// Provisioning + verrouillage de l'outil de rendu Rainmeter.
// TOUTE la logique pure (décision provisioning install-if-absent,
...
//
// Contrainte forte projet « handler jamais installeur » :
// le provisioning est appelé au
// BOOTSTRAP du cycle SYSTEM, JAMAIS depuis un handler runtime. Rainmeter est
// extrait en mode PORTABLE (zéro registre, zéro MSI/NSIS/winget).
//
// Aucune dépendance AD/LDAP/APCu ici — pose d'un
// asset vérifié + maintien, rien d'autre.

const (
	// L'intégrité ET le nom du portable viennent désormais
	// du MANIFEST tool/skin servi (GET /api/v1/agent/tools-manifest), plus d'une
	// constante figée dans le binaire (`RainmeterToolFilename` /
	...
	// invariant). La skin suit le même chemin : téléchargée
	// (vérif SHA-256) depuis GET /api/v1/agent/overlay-skin, plus d'embed
	// go:embed.

	// RainmeterToolsManifestRoute : endpoint du manifest tool/skin DÉDIÉ,
	...
	// rainmeterInstalledMarker : marqueur d'extraction COMPLÈTE. Écrit en
	...
	// l'archive entière — défense zip-bomb complémentaire de la borne par
...
// ProgramData ACL est le point de verrouillage). Racine paramétrable
...
// InstalledMarkerPath : marqueur d'extraction complète, écrit en dernier.
...
// Sentinelle d'idempotence du provisioning (install-if-absent). On se
...
// le marqueur par provisionRainmeterPortable (autorité serveur). C'est la
...
// ProgramData) que le Rainmeter.ini durci déclare via SkinPath.
...
// pas). Contenu déterministe (pas de champ volatil) → idempotence
...
//     Rainmeter).
//
// IMPORTANT : WindowX/WindowY dans le Rainmeter.ini de settings sont
...
func BuildHardenedRainmeterIni() string {
	lines := []string{
		...
		"Debug=0",
		// MODE INSTALLÉ : ce Rainmeter.ini vit désormais en
...
// (%APPDATA%\Rainmeter\Rainmeter.ini) en UTF-16 LE + BOM, prêt à écrire (mode
// installé). Pivot PUR/déterministe testé sur l'hôte : c'est le
...
// est partagée. UTF-16 LE + BOM pour homogénéité Rainmeter.
...
// rester WRITABLE (c'est tout l'objet de la — propriété naturelle
...
// VOULU (réimposition du durci à chaque logon, l'élève ne fige pas une
...
func ExtractPortableZip(archive []byte, destDir string) error {
	...
	// Compteur de bytes TOTAUX décompressés : borne cumulée en plus de la
...
// bytes écrits (pour la comptabilité de la borne totale).

...
// Logique PURE, OS-agnostique (les accès registre réels sont
// injectés via RegistryOps) → testée sur l'hôte ; agent/windows ne fait que
// câbler golang.org/x/sys/windows/registry.
//
// UN SEUL handler générique pour les DEUX portées : le SERVICE SYSTEM
...
//      type `registry_list`). Déjà absente = compliant (idempotent) ;
...
//      clé hors cible.
//
// ISOLATION des erreurs : une clé protégée / ruche absente → l'op renvoie
...
//
// RUCHE `HKU` (contrat §7.1) — FAN-OUT interne au handler : un item
...
// Verbe `ensure` du contrat §7.1 : optionnel, absence = present.
const (
...
type RegistrySpec struct {
	...
	// Ensure : verbe de convergence. "" ou "present" = écrire la
	// valeur cible ; "absent" = supprimer la valeur nommée (Value est vide).
	Ensure string
	Value  RegistryValue
	// Refresh : hint OPTIONNEL du payload (champ `refresh`) — le
	// geste de rafraîchissement requis si CET item change effectivement.
	// Lecture indulgente : absent/vide/inconnu = RefreshNone, jamais une
	// enveloppe invalide. Un hint ne peut qu'ESCALADER le plancher shell_notify
	// des changements HKCU, jamais l'affaiblir.
...
type RegistryOps interface {
	...
	// (l'item `absent` doit la voir pour la supprimer). err =
	...
	// depuis l'énumération (race logoff) : no-op nil — JAMAIS
	...
	// Delete supprime la VALEUR NOMMÉE d'une clé — JAMAIS la
	...
	// UserHives énumère les CIBLES du fan-out HKU : les sous-clés
...
// NB : UserHives est un op REQUIS de l'interface — contrairement
...
// passe. Une seule voie d'émission (plus jamais deux
// SHChangeNotify pour un même changement).

// isUserHive : la ruche est-elle celle de l'utilisateur (HKCU) ? Gate
// l'accumulation du besoin de rafraîchissement (échelle — plancher
...
// (branche default) : le service écrit depuis la
...
// IsUsersHive : la ruche est-elle HKEY_USERS (HKU) ? Gate le
...
// passe), JAMAIS retenue entre deux cycles (une session ouverte
...
// préfixe `.DEFAULT\` lui-même (un path de seed commençant par
...
type RegistryHandler struct {
	...
	// la passe courante — max(plancher shell_notify, hint) de
	...
	// même code) n'accumule jamais (gate isUserHive — HKLM/HKU rendent false)
...
// logHints : la trace du hint `refresh` inconnu n'est émise
...
// Item HKU : drift AGRÉGÉ — conforme ssi TOUTES les cibles du
...
// DESIGN ASSUMÉ : les erreurs de LECTURE restent franches en
...
// quand même, isolation inter-items). Ne supprime/efface JAMAIS une clé
// absente de la cible (« ne pas gérer » = ne pas toucher).
// Item HKU : converge CHAQUE cible du fan-out en effort maximal
...
func (h *RegistryHandler) Apply(items []StateItem) error {
	...
	for _, spec := range specs {
		...
		for _, target := range targets {
			...
			// ACCUMULE le besoin de rafraîchissement : plancher
			// shell_notify (iso-comportement du SHChangeNotify historique),
			...
			// HKU : jamais (isUserHive rend false). Le geste
			// lui-même est exécuté par le COMPAGNON en fin de RunPass
			// (RefreshRequester) — plus d'émission inline ici :
...
// error. `name` (scope) : la clé DOIT être PRÉSENTE dans le
...
// Verbe `ensure` (optionnel) : `"absent"` ⇒ item de
...
func parseRegistrySpec(raw any) (RegistrySpec, bool) {
	...
	// Hint `refresh` OPTIONNEL — lecture INDULGENTE :
	// absent, vide, non-string ou vocabulaire inconnu ⇒ RefreshNone,
	...
	// Branche `ensure` AVANT l'exigence type/value :

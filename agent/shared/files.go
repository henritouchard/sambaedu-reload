...
// Contrats locaux du poste (FIGÉS — docs/agent/enrollment.md §3,
...
const (
	...
	// au fallback overlay commun quand le profil de
	...
	// : une ACE NTFS ne porte AUCUN marqueur de propriété — ce
	...
	// ProgramData\SambaEdu\Agent déjà ACL SYSTEM+Administrators (dogme).
	fsAclStateFile = "fsacl-state.json"
	logsDirName    = "logs"

	// DefaultIntervalSeconds : cadence par défaut, iso ttl_seconds
	// conseillé par le serveur.
	DefaultIntervalSeconds = 3600

	// WpkgBundlePath : sous-chemin (relatif à server_url) du bundle WPKG
	// pré-substitué servi en STATIQUE par Apache (PAS via
	...
	// servant les OUTILS PARTAGÉS WPKG (7za.exe, nircmd.exe, … —),
	...
	// réveil au logon. Un réveil logon ne déclenche un cycle frais
...
// décision conservée) et ACL posée à la création via SetACL.
//
// SetACL est injecté par le binaire Windows (shell-out icacls.exe :
...
// TokenPath : chemin du token — CONTRAT FIGÉ, ne jamais en dériver.
...
// fs_acl — <root>\fsacl-state.json, écrit ATOMIQUEMENT
...
// l'avoir écrit). 64 hex sans newline (contrat) ; les espaces/newline
// parasites sont trimés avant validation.
...
// valider (un token malformé « existe » au sens de ce prédicat) :
...
// ré-enrôle JAMAIS automatiquement).
...
// WriteToken écrit le nouveau token ATOMIQUEMENT : 64 hex sans
...
// VIDE (`{}`) s'il n'existe pas — infrastructure du mode `default`
// préservée pour les handlers.
...
// format CONSERVÉ : {"server_url": "...", "interval_seconds": 3600}.
type Config struct {
...
// interval_seconds optionnel (défaut 3600 s).
...
// (ex. profils WPKG par-hôte —) pour maintenir la cohérence de

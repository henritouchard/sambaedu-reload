# `POST /api/v1/agent/report` — l'ingestion des rapports de conformité

> **Le retour** du canal agent : comment l'agent rapporte sa conformité,
> item par item, et ce que le serveur en stocke. Orthogonal à
> [contract-v1.md](contract-v1.md) (qui fige le wire format du rapport — §6 +
> golden `tests/Fixtures/Agent/report.v1.json`) et à
> [state-endpoint.md](state-endpoint.md) (l'aller, l'état désiré servi au poste).
...
| Auth | bearer token per-poste — **l'identité de la requête EST le poste résolu par le token**, jamais le payload |
...
| Corps 200 | ACK serveur→agent : `{"success": true, "counts": {"compliant": n, "drift": n, "error": n}}` — l'agent n'a besoin que du 2xx |
| Écritures | tables `agent_*` (`ReportIngestService`) — `agent_last_checkin_at` et la rotation restent gérés par le middleware `agent.token` |
...
Diagnostic mainteneur (poste enrôlé, sans agent installé) :
...
## Identité = le token, jamais le payload
...
travail du middleware `agent.token`, pas du report.

## Stockage — trois tables, volume borné
...
| `agent_report_history` | payloads bruts complets, append-only, **debug** | flag `AGENT_REPORT_HISTORY` (défaut **off**) + rétention `report_history_retention_days` (30 j) |

`reported_at` de la ligne d'état est rafraîchi à **chaque** rapport, même
identique (fraîcheur du dernier rapport = donnée UI) — « identique »
n'inhibe que la création d'ÉVÉNEMENT.

### Règle de création d'événement
...
NAN/INF) produit un 4xx, jamais un 500.

### Solde d'une demande « forcer la synchro »
...
l'UI parc — voir [state-endpoint.md](state-endpoint.md) § « Forcer la
synchro ») : `agent_sync_requested_at` est remis à **null**, un log
...
transaction d'ingestion (colonne `agent_*`, idempotente). Une demande sur
un poste en **quarantaine** ne sera jamais soldée (403 au report) — l'UI
désactive le bouton pour ces postes.

## Conformité — `ConformityService`

`ReportIngestService` délègue à `ConformityService` la lecture de l'état
courant agrégé d'un poste à partir de `agent_resource_states` : c'est cette
vue qui alimente l'UI parc (un poste est conforme quand tous ses items ont
`status = compliant`).
...
| 401 | bearer absent (`AGENT_TOKEN_MISSING`) ou invalide (`AGENT_TOKEN_INVALID`) | `{error, message, code}` |
| 403 | poste en quarantaine (`AGENT_QUARANTINED`) — **aucune** écriture de rapport | `{error, message, code}` |
| 422 | rapport malformé : `schema` inconnu, `items` absent/mal typé, `status` hors enum, `hash` non hex-64, `detail` absent/vide sur `error`, `type` hors liste publiée (§7 du contrat) ou dupliqué — **rien n'est écrit** | `{message, errors}` (ValidationException Laravel) |
| 429 | throttle 60/min/IP dépassé | réponse Laravel standard (hors contrat) |

Champs **inconnus ignorés** (règle d'évolution §9 du contrat : champ ajouté =
version mineure — le serveur tolère l'inconnu).
...
- [state-endpoint.md](state-endpoint.md) — l'aller (état désiré servi au poste).

...
- [ ] [agent](domains/agent.md) — Canal agent desired-state : ingestion des rapports POST /report, stockage D3 borné, agent squelette + compagnon de session, handlers wallpaper/overlay + route assets (démo palier 1), agent Go core/service SYSTEM/build signé, compagnon + handlers Go parité démo (binaire complet 2.1.0, zéro .ps1) _(Stories 24.1 → 24.6)_

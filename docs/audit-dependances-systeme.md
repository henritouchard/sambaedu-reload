...
| `includes/config.inc.sh` (+ `utils.inc.sh`) | `sambaedu-config` | sourcé par `make_dhcpd_conf.sh` et les autres scripts sbin | ❌ — reste [A], traité en Vague 1 (config auto-portée). **BLOQUANT greenfield DHCP dur** (confirmé Henri 2026-07-11) : `make_dhcpd_conf.sh` versionné par 8.3 fait `. config.inc.sh` en tête et lit toutes les `config_*` via ce parseur — sans lui, échec silencieux (variables vides, aucun `subnet{}` généré). AC6 « greenfield-proof » de 8.3 est donc sciemment restreint aux 2 scripts DHCP ; un déploiement sans paquets SE4 exige de porter aussi `config.inc.sh` (Vague 1). |

 ...
 */
class WorkstationGroupPolicyCustomizeTest extends TestCase
{
    ...
    public function delegate_with_positive_grant_on_a_is_allowed_on_a_and_denied_on_b(): void
    {
        ...
        app(PermissionService::class)->grantDelegation($delegate, 'app.customize', $salleA);

        $this->assertTrue($this->policy->customize($delegate, $salleA), 'AC#1 : autorisé sur sa salle');
        $this->assertFalse($this->policy->customize($delegate, $salleB), 'AC#1 : refusé hors périmètre');
    }
    ...
    public function global_admin_is_allowed_everywhere_including_null_scope(): void
    {
        ...
        $this->assertTrue($this->policy->customize($admin, $salleA), 'AC#2 : global → A');
        $this->assertTrue($this->policy->customize($admin, $salleB), 'AC#2 : global → B');
        $this->assertTrue($this->policy->customize($admin, null), 'AC#2 : global → scope null (find inexistant)');
    }

    #[Test]
    public function negative_exclusion_prevails_even_over_global_right(): void
    {
        $user = $this->makeUser('mixte', ['app.customize']);
        $salleA = $this->makeGroup('salle_a');

        app(PermissionService::class)->negateDelegation($user, 'app.customize', $salleA);

        $this->assertFalse(
            $this->policy->customize($user, $salleA),
            'AC#3 : exclusion négative active prévaut même sur le droit global'
        );
    }
    ...
    public function expired_delegation_is_denied(): void
    {
        ...
        $this->assertFalse(
            $this->policy->customize($delegate, $salleA),
    ...
    public function user_without_anything_is_denied(): void
    {
        ...
        $this->assertFalse($this->policy->customize($lambda, $salleA));
        $this->assertFalse($this->policy->customize($lambda, null));
    }
    ...
    public function logical_group_falls_back_to_global_only(): void
    {
        ...
        $admin = $this->makeUser('admin', ['app.customize']);
        $this->assertTrue($this->policy->customize($admin, $logical), 'global → autorisé sur groupe logique');
        ...
        app(PermissionService::class)->grantDelegation($delegate, 'app.customize', $logical);
        $this->assertFalse(
            $this->policy->customize($delegate, $logical),
    ...
    public function registered_gate_resolves_through_facade(): void
    {
        ...
        // bien le Gate nommé que `guardCustomize()` invoque.
        $delegate = $this->makeUser('delegate');
        $admin = $this->makeUser('admin', ['app.customize']);
        $salleA = $this->makeGroup('salle_a');
        $salleB = $this->makeGroup('salle_b');
        ...
        $this->assertTrue(Gate::forUser($delegate)->allows('customize-workstationGroup', $salleA));
        $this->assertFalse(Gate::forUser($delegate)->allows('customize-workstationGroup', $salleB));
        $this->assertTrue(Gate::forUser($admin)->allows('customize-workstationGroup', $salleB));
        $this->assertTrue(Gate::forUser($admin)->allows('customize-workstationGroup', null));
    }
}

...
function poursuivre() {
    echo
    reponse=""
    while [ "$reponse" != "o" -a "$reponse" != "O" -a "$reponse" != "n" ]; do
        echo -e "$COLTXT"
...
function check_proxy() {
    ...
    if curl --connect-timeout 3 -s https://sambaedu.org >/dev/null; then
        cnx_https="ok"
    else
        if [ -n "$proxy_address" ]; then
...
* Les domaines AD du type .lan ou .local sont à proscrire"
        ...
        domain="$(hostname -d)"
        while [ "$samba_domain_check" != "ok" ]; do
            color="color15"
...
# Fonction écriture fichier de conf sambaedu
function write_sambaedu_ws_config() {
    cat >$se4fs_config <<END
## Params du futur SE4-AD ##
## Adresse IP du futur SE4-AD ##
se4ad_ip="$se4ad_ip"
...
function debconf_ws_set() {
    echo "sambaedu-ad-dc sambaedu/type_ad text Installer se4fs domaine Windows Server
sambaedu-boot-server sambaedu/dhcp_external boolean true
sambaedu-ad-dc sambaedu/ad_os text Windows Server
sambaedu-ad-dc sambaedu/ldap_port text 389
" | debconf-set-selections
}

<?php
/*
 * génération d'un fichier autorun qui lancera les actions sur le debian au boot, récupere le nom et le uuid en GET
 * execute en boucle le script généré par linux/action.php
 */
...
do
    ...
    scripts=$(curl -F \"uuid=" . $uuid . "\" -F \"name=" . $machine['cn'] . "\" -F \"ret=\${ret}\"  -s \"" . $config['ipxe_url'] . "linux/action.php\")
    eval \"\$scripts\"
    ret=\$?
    if [ \"\$ret\" -eq \"0\" ];
    then
...
# en mode daemon, on ne doit jamais quitter !
        exit 1
    fi
    sleep 1
done
";
    ...
    $autorun .= file_get_contents("/usr/share/sambaedu/includes/utils.inc.sh");
    $autorun .= "
echo 'pas de mode daemon sambaedu pour cette machine!'
exit 1
";
}
echo $autorun;

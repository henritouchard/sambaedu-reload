    ...
    if (count($machine) > 0) {
        ...
        if (isset($_POST['parc'])) {
            ...
            if (in_array($parc, array_column($parcs, "samaccountname"))) {
                $ipxe .= "echo La machine est deja dans le parc " . $parc . "\n";
                $ipxe .= "sleep 3\n";
            } else {
                $res = add_member_parc($config, $parc, $machine['cn']);
                if ($res) {
                    $ipxe .= "echo La machine a ete ajoutee au parc " . $parc . "\n";
                    $ipxe .= "sleep 3\n";
                } else {
                    $ipxe .= "echo ERREUR La machine n'a pas ete ajoutee au parc " . $parc . "\n";
                    $ipxe .= "sleep 3\n";
                }
            }
        } else {
            $ipxe .= "menu Enregistrement d'un parc pour " . $machine['cn'] . "\n";
            ...
            foreach ($dispo as $p) {
                if (in_array($p['samaccountname'], array_column($parcs, "samaccountname"))) {
                    $ipxe .= "item fin ** deja dans " . $p['name'] . " **\n";
                } else {
            ...
            $ipxe .= "chain --replace --autofree parcs.php##params\n";
        }
    }
}
$ipxe .= ":fin\n";

...
- **`rename` cross-device (review S1) — CORRIGÉ** : `rename()` échoue `EXDEV` si `/home/profiles` et la corbeille sont sur des mounts distincts (config prod fréquente : volume profils dédié). Méthode `moveToTrash()` ajoutée : tente `rename()`, et sur échec bascule sur `mv -f` (gère le cross-device par copie+suppression) ; succès confirmé seulement si la source a disparu. Repli cantonné au chemin d'action admin de la purge (jamais au render → invariant perf préservé, vérifié par `it_never_shells_out_at_render_or_purge`). Test `it_falls_back_to_mv_when_rename_fails_cross_device`. Collision de suffixe horodaté (même seconde) corrigée par un suffixe `-N` incrémental. Décision Henri 2026-06-15 : robustesse appliquée tout de suite.

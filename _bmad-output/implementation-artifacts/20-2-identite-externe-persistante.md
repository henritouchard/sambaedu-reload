...
4. **Base légale de rétention RGPD (IR-M5) — cœur de la story** :
   ...
   - **Commande planifiée** `php artisan federated:purge-identities` **calquée sur `trash:purge`** (`app/Console/Commands/TrashPurgeCommand.php`) : `--dry-run`, `--force`, sélectionne les `ExternalIdentity` dont `last_login_at < now - pii_ttl_days` (**critère temporel SEUL** — décision P-7 / Q-4 review 20.2 : pas de filtre « User actif rattaché ». Exclure une identité encore liée à un `User` actif créerait un risque RGPD **inverse** — PII conservée au-delà du TTL ; et `last_login_at` étant rafraîchi à chaque login, une activité réelle sort déjà naturellement de la fenêtre. Le hold/exclusion légal explicite est une action admin future — 20.3), **anonymise** (pas hard-delete), trace chaque anonymisation (log channel `federated-auth` + ligne d'audit best-effort). Fail-soft (un échec n'arrête pas la boucle). No-op safe si `anonymize_enabled=false` ou TTL ≤ 0 (warning + exit clair, cf. garde-fou `trash:purge`).
...
- `app/Http/Middleware/Auth/SambaEduAuthGuard.php` (M-2 : `subForLog()` hashe le `sub` dans les 2 logs fédérés, gère préfixe `anon:` ; 3e dépendance `ExternalIdentityLifecycleService`)
- `tests/Feature/AuthGuardInterfaceTest.php` (3e arg du constructeur guard)
...
- 2026-06-02 — Corrections post-review (review sonnet + 2e avis opus) : P-1 (`--dry-run` énumère même toggle OFF), P-2/M-4 (`anonymize()`/`softDeleteWithReason()` dans `DB::transaction()`), P-8 (motifs tronqués 255). Puis 4 décisions Henri appliquées : P-4 (HMAC clé dédiée `retention.hash_key` au lieu de sha256 nu), P-7 (purge temporel seul confirmé, doc alignée), M-2/P-9 (guard `subForLog()` hashe le sub + gère `anon:`), Q-1 (TTL 365j validé, texte base légale à relire DPO). 77 tests verts (federated+console+guard). Doc review `codeReviews/20-2.md` → done.

...
9. **MAJ post-cycle (2026-06-12, commit 6cec1af)** : les notes 2 et 3 sont SOLDÉES — (a) émission du PFX code-signing **automatisée** (`scripts/emit-codesign-pfx.sh`, idempotent, câblé `update.sh::ensure_codesign_pfx`, ré-émission forcée si CA régénérée) ; (b) **build signé automatisé côté serveur** (`scripts/build-agent.sh` : toolchain Go 1.26.4 épinglée SHA-256 dans /usr/local/go + osslsigncode amorcés, no-op si dist/ à jour, câblé `update.sh::ensure_agent_build` — le PFX ne quitte plus le serveur) ; binaire `dist/sambaedu-agent-2.0.0.exe` rebuilé sur la VM signé **CA réelle** (verify ok) ; (c) fantômes `.ps1` purgés de la VM (8 fichiers, vérifiés absents du repo avant rm). **Seul restant : T11 install lab ws 49.**

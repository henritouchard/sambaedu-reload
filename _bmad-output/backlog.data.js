      ...
      "stories": [
        {
          ...
          "status": "done",
          "note": "[DONE — tracking main ; code+story sur branche worktree-E34-lecteurs, PAS encore mergé sur main] Fondation backend (décision Henri : pas d'UI, pas de templates) : table network_shares + pivot polymorphe network_share_assignables (User|UserGroup|WorkstationGroup, access ro|rw) ; NetworkShareService (provisioning /var/sambaedu/Partages + ACL POSIX rx/rwx, WG=montage-seul) ; extension DrivesStateProvider (émet les répertoires configurés par maille en plus de K:/H:, payload {letter,unc,label} inchangé). Golden state figé préservé quand 0 ligne ; zéro modif agent."
        },
        {
          "id": "34-2",
          "title": "UI admin (refnum) des lecteurs réseau gérés",
          "status": "done",
          "note": "[DONE — tracking main ; code+story sur branche worktree-E34-lecteurs, PAS encore mergé sur main] IMPLÉMENTÉE 2026-06-30 (dev-story, Opus 4.8) → review. Page liste /app/shares (SFC Volt, recherche/pagination, x-organisms.page+data-table) ; modale de création x-molecules.modal (name, directory_name validé FORMAT via NetworkShareService::DIRECTORY_NAME_PATTERN + unique [M4], label, letter pré-remplie prochaine libre [Q2] refusant les réservées [#4]) ; page détail /app/shares/{id} (édition + assignation par maille pivot SQL pur User/UserGroup/WorkstationGroup access ro|rw zéro CN AD, upsert unicité, re-provision) + suppression (cascade pivot, FS conservé 34.x) ; provision() SYNCHRONE→toast [Q1]. NetworkShareValidator (pure lecture, calquée 30.5) : WG-montage-seul=warning [M5], collision lettre explicite+audience recouvrante=NetworkShareLetterCollisionException bloquante [M1/piège #3], lettre réservée=erreur. POLICY DÉDIÉE NetworkSharePolicy + permissions networkshare.view/manage [Q5] (ReferentNumerique+ShareAdmin+UserAdmin+SuperAdmin auto, isSecondaryBitPermission → refnum a networkshare.manage mais PAS share.manage). GARDE-FOUS PROUVÉS : golden state.v1.json + FROZEN_STATE_HASH PHP INCHANGÉS, agent/**/StateCompiler/ShareService/provision INTOUCHÉS (seules modifs : RESERVED_LETTERS private→public + const DIRECTORY_NAME_PATTERN), pas de bump agent, zéro AD/APCu. Tests HÔTE : 31 net-new (Validator 12/Policy 4/SharesIndex 7/ShareDetail 8), non-régression Agent 540/22skip + ContractV1 5 + enums/rôles 81 + RoutesProtection 28. ⚠️ VM : db:seed --class=PermissionSeeder requis. DETTE documentée : pickers non scopés par établissement [Q3]. --- ANTÉRIEUR (create-story) : Couche UI Livewire + validation prédictive PAR-DESSUS la fondation 34.1 (réutilisation stricte NetworkShare/pivot/NetworkShareService::provision/DrivesStateProvider). LIVRE : page liste /app/shares (SFC Volt, recherche/pagination, scope SharePolicy) ; création/édition (modale réutilisable x-molecules.modal — name, directory_name validé au FORMAT [finding 34.1 M4] + unique, label, letter explicite-ou-auto refusant les lettres réservées K/H/I/L/A-D [finding 34.1 #4 : RESERVED_LETTERS exposée public]) ; assignation par maille au pivot SQL pur (User/UserGroup/WorkstationGroup, access ro|rw — PAS de CN AD) + re-provision ; déclenchement provision() synchrone→toast (WithToasts) ; VALIDATION PRÉDICTIVE (cœur de valeur, calquée UpstreamLockCollisionDetector 30.5, pure lecture) : WG-montage-seul=warning non bloquant [piège #1/finding M5], collision de lettre entre 2 répertoires à audience recouvrante=erreur bloquante [piège #3 34.1/finding M1], lettre réservée=erreur. GARDE-FOUS : payload drives/golden state.v1.json/FROZEN_STATE_HASH PHP+Go/agent/**/StateCompiler/contract-v1 §7/ShareService/provision INTOUCHÉS (seule modif provider = RESERVED_LETTERS private→public). HORS scope : templates de répertoire (34.3), resync/archivage/suppression FS (34.x). 7 AC, T1-T7. Questions ouvertes Henri : Q1 provision sync vs job (défaut sync), Q2 lettre stable encourager vs imposer (défaut encourager), Q3 scope pickers user/groupe en zone grise SQL/AD (défaut pickers non scopés + dette), Q4 foyer RESERVED_LETTERS (défaut public const provider), Q5 policy share.* réutilisée vs NetworkSharePolicy dédiée. CRÉÉE 2026-06-30 (create-story, Opus 4.8) → ready-for-dev. Reco dev : OPUS (validation prédictive type-30.5 + préservation contrat agent figé + 5 arbitrages ouverts, pas le CRUD)."
        },
        {
          "id": "34-3",
    ...
    },
    {
      ...
      "stories": [
        ...
        },
        {
          ...
          "status": "review",
          "note": "[GATE D6 RÉOUVERT (Henri 2026-07-04) → dev fable → review opus → APPROUVÉ, commit main] Réutilise la plomberie Epic 36 (coût marginal effondré). Livré : type contrat additif privilege (exclusive, Machine, {privilege, accounts}), provider PrivilegeCapabilityProvider (AudienceTokens réutilisé), guard PrivilegeAuthoringGuard (SeDeny*-only + borne de PORTÉE des accounts : principals larges refusés serveur+agent, correction review #1), handler Go conteneur SANS store (iso firewall) + impl Win32 LSA (isBroadPrincipalSid sur SID résolu), seed rdp_denied_for_group (off réel), golden +1 item machine (e87fed16…), agent 2.7.0→2.8.0. Review opus 5 problèmes tous corrigés (#1 portée décision Henri ; #2 warning secpol ; #3 LsaFreeMemory nil ; #4 log symétrique ; #5 commentaire). Tests PHP 54 / Go complet + vet/build windows OK. Runbook QA agent.md §35.6 (7 scénarios). Doc review 35-6.md. Restent (opérateur) : publier release 2.8.0 (livre fs_acl+firewall+privilege) + rejouer migration seed /vm + e2e lab. Validation Henri : review→done."
        }
      ]
    },
    {

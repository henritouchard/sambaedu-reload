 ...
 */
/*******************************************************************************
 * Version de wpkg.js modifiée pour SambaEdu4
 *
 * PATCH pour SE3 (Olivier Lacroix)
 * Les modifs par rapport à wpkg.js officiels sont délimitées par // PATCH SE4 et // FIN PATCH SE4 :
...
var se4fs = wshShell.ExpandEnvironmentStrings("%se4fs%");
if (se4fs == null) {
	se4fs = "se4fs";
}
...
/**
 * var packages_file_name = "packages.xml"; /** name of profiles database file
 */
/**
 * var profiles_file_name = "profiles.xml"; /** name of hosts definition
 * database file
 */
/**
 * var hosts_file_name = "hosts.xml"; // FIN PATCH SE4
 *
...
/**
 * Allows to disable inserting of host attributes to local settings DB. This is
 ...
 * attributes are present. This setting might be removed if all test-cases are
 * adapted.
 */
 ...
 * another (remote-) system you can set queryMode to "remote". This will disable
 * package checks because they will not return the same results on a remote
 * system.
 */
var queryMode = "local";
 ...
 * continue execution. NOTE: You can override the default timeout using the
 * timeout=xxx attribute on command definition. It is strongly recommended to
 * keep the default as-is and just define custom timeouts on commands which are
 * known to finish within a shorter timeframe or of course if commands are known
 * to run longer than the default timeout.
 */
 ...
 * is used to prevent package dependencies without checks and "always execute"
 * to be executed several times as well as preventing infinite- loops on
 * recursive package installation.
 */
...
/**
 * stores the host locale ID of the OS (LCID) which applies for the local
 * machine
 */
var LCIDOS = null;
 ...
 * only!
 */
var volatileReleaseMarkers = ["rc", "i", "m", "alpha", "beta", "pre",
	"prerelease"];
...
function main() {
	...
	setQuiet(true);
	// PATCH SE4 : oubli dans le script officiel.
	if (!isNoRunningState()) {
	...
	if (argn.Item("query") != null) {
		...
		if (listSyncInstall || listSyncUpgrade || listSyncDowngrade
			|| listSyncRemove || listSyncUnmodified) {
			queryProfilePackages(listSyncInstall, listSyncUpgrade,
				listSyncDowngrade, listSyncRemove, listSyncUnmodified);
		}
		...
		if (isDebug()) {
			...
			message = "Settings file contains " + settingsPkg.length
				+ " packages:";
			...
			message = "Packages file contains " + packageNodes.length
				+ " packages:";
			...
			message = "Profile file contains " + profileNodes.length
				+ " profiles:";
		...
		if (argn.Item("show") != null) {
			...
			// if case sensitive mode is disabled convert package name to lower
			// case
			if (!isCaseSensitive()) {
				requestedPackageName = requestedPackageName.toLowerCase();
			}
			var message = queryPackage(
				getPackageNodeFromAnywhere(requestedPackageName), null);
			info(message);
...
function addSettingsNode(packageNode, saveImmediately) {
	...
	if (settingsNode != null) {
		dinfo("Removing currently existing settings node first: '"
			+ getPackageName(settingsNode) + "' ("
			+ getPackageID(settingsNode) + "), Revision "
			+ getPackageRevision(settingsNode) + ".");
		removeSettingsNode(settingsNode, false);
	}

	dinfo("Adding settings node: '" + getPackageName(packageNode) + "' ("
		+ getPackageID(packageNode) + "), Revision "
		+ getPackageRevision(packageNode) + ".");
...
function appendProfileDependencies(profileArray, profileNode) {
	...
	for (var i = 0; i < profileNodes.length; i++) {
		...
		if (!searchArray(profileArray, currentNode)) {
			...
			dinfo("Profile '" + getProfileID(currentNode) + "' "
 ...
 *
 * @param checkNodes
 *            Array of XML <check /> nodes to be evaluated.
...
function checkCondition(checkNode) {
		...
		if (checkType != "logical") {
			...
			if (result == null) {
				error("Result of check of type '"
					...
					+ "' and value '"
					+ checkValue
					+ "' is missing in settings database. "
	...
	if (checkType == null) {
		throw new Error(
			"Check Type is null - this is not permitted. Perhaps a typo? "
	...
	switch (checkType) {
		...
		case "registry":
			...
			if ((checkCond == null) || (checkPath == null)) {
				throw new Error(
					"Condition and / or path is null for a registry check. Perhaps "
					+ "a typo? To help find it, here are the other pieces of information: "
					+ "condition='" + checkCond + "', path='"
					+ checkPath + "', value='" + checkValue + "'.");
			}
			...
			switch (checkCond) {
				case "exists":
					if (getRegistryValue(checkPath) != null) {
						...
						dinfo("The registry path '" + checkPath
							+ "' exists: the check was successful.");
						returnValue = true;
					} else if (getRegistryValue(checkPathExpanded) != null) {
						dinfo("The expanded registry path '" + checkPathExpanded
							+ "' exists: the check was successful.");
						returnValue = true;
					} else {
						// path does not exist
						dinfo("Neither the registry path '" + checkPath
							+ "' nor its expanded value of '" + checkPathExpanded
				...
				case "equals":
					...
					if (registryValue == checkValue) {
						// Some debugging information.
						dinfo("The registry path '" + checkPath
							+ "' contained the correct value: '" + checkValue
							+ "'. The check was successful.");
						returnValue = true;
					} else {
						// Try if expanded value matches (case-insensitive).
						if (registryValue.toLowerCase() == checkValueExpanded
							.toLowerCase()) {
							dinfo("The registry path '" + checkPath
							...
							try {
								var registryValueMatcher = new RegExp(
									checkValueExpanded, "i");

								if (registryValueMatcher.test(registryValue) == true) {
									dinfo("The registry path '"
										+ checkPath
										+ "' value matched the regular expression: '"
										+ checkValueExpanded
										+ "'. The check was successful.");
									returnValue = true;
								} else {
								...
								dinfo("Registry check value '" + checkValueExpanded
									+ "' cannot be used as "
									+ "regular expression.");
							}
							...
							if (returnValue != true) {
								dinfo("The registry path '" + checkPath
									+ "' did not contain the value: '" + checkValue
									+ "'. Instead it contained '" + registryValue
									+ "'. the check failed.");
							}
						}
					}
		...
		case "file":
			...
			if ((checkCond == null) || (checkPath == null)) {
				throw new Error(
					"Condition and / or path is null for a file check. Perhaps "
					+ "a typo? To help find it, here are the other pieces of information: "
					+ "condition='" + checkCond + "', path='"
					+ checkPath + "', value='" + checkValue + "'");
			}
			...
			if (checkCond == "exists") {
				...
				if (fso.FileExists(checkPath)) {
					// Some debugging information.
					dinfo("The path '" + checkPath
						+ "' exists and is a file: the test was successful.");
					returnValue = true;
				} else if (fso.FolderExists(checkPath)) {
					// Some debugging information.
					dinfo("The path '" + checkPath
						+ "' exists and is a folder: the test was successful.");
					returnValue = true;
				} else {
					// Some debugging information.
					dinfo("The path '" + checkPath
						+ "' does not exist: the test failed.");
					returnValue = false;
				}
				...
				if (checkValue == null) {
					throw new Error(
						"Value is null for a file sizeequals check. Perhaps "
						+ "a typo? To help find it, here are the other pieces of information: "
						+ "condition='" + checkCond + "', path='"
						+ checkPath + "', value='" + checkValue + "'.");
				}
				...
				if (filesize == checkValueExpanded) {
					dinfo("The file '" + checkPath + "' has size " + filesize
						+ ": the test was successful.");
					returnValue = true;
				} else {
					dinfo("The file '" + checkPath + "' has size " + filesize
						+ " - wanted " + checkValueExpanded
						+ ": the test fails.");
					returnValue = false;
				}
				...
				if (checkValue == null) {
					throw new Error(
						"Value is null for a file version check. Perhaps "
				...
				if (fileVersion == null || fileVersion == "") {
					...
					var fileVersionCompare = versionCompare(fileVersion,
						checkValueExpanded);
					dinfo("Checking file version " + fileVersion + " is "
						+ checkCond + " (than) " + checkValueExpanded
						+ " - got result " + fileVersionCompare + ".");
					...
					dinfo("File version check for file '" + checkPath
						+ "' returned " + fileVersionCompResult
						+ " for operation type " + checkCond + ".");
				...
				if (dateType == "modify") {
					...
					throw new Error("Invalid file date comparison type: "
						+ checkCond + ".");
				}
				...
				if (firstChar == "+" || firstChar == "-") {
					...
					dinfo("Reading relative comparison date: " + checkValueExpanded
						+ " minutes.");
					...
					if (dateType == "modify") {
						dinfo("Reading file modification date of reference file '"
							+ filePath + "'.");
						// Evaluate file modification date.
						comparisonDate = getFileDateModification(filePath);
					} else if (dateType == "create") {
						dinfo("Reading file creation date of reference file '"
							+ filePath + "'.");
						// Evaluate file creation date.
						comparisonDate = getFileDateCreation(filePath);
					} else if (dateType == "access") {
						dinfo("Reading file access date of reference file '"
							+ filePath + "'.");
						// Evaluate file access date.
				...
				if (comparisonDate == null) {
					throw new Error("Unable to evaluate date from value '"
						+ checkValueExpanded + "'.");
				}
				...
				switch (comparison) {
					...
					case "equalto":
						...
						// Reduce accuracy to milliseconds for equal comparison when
						// comparing to user string.
					...
					default:
						throw new Error("Invalid file date comparison parameter: '"
							+ checkCond + "'.");
						break;
				}

				if (success) {
					dinfo(comparisonDesc + " date of file '" + checkPath + "' is "
						+ fileDate + " which is " + comparisonCond
						+ " the comparison date " + comparisonDate
						+ " check succeeded.");
				} else {
					dinfo(comparisonDesc + " date of file '" + checkPath + "' is "
						+ fileDate + " which isn't " + comparisonCond
						+ " the comparison date " + comparisonDate
						+ " check failed.");
				}
				returnValue = success;
		...
		case "uninstall":
			...
			if ((checkCond == null) || (checkPath == null)) {
				throw new Error(
					"Condition and / or path is null for an uninstall check. Perhaps "
					+ "a typo? To help find it, here are the other pieces of information: "
					+ "condition='" + checkCond + "', path='"
					+ checkPath + "'.");
			}
			...
			if (checkCond == "exists") {
				...
				if (checkValue == null) {
					throw new Error(
						"Uninstall entry version check has been specified but no"
					...
					for (var iUninstKey = 0; iUninstKey < uninstallLocations.length; iUninstKey++) {
						...
						if (uninstallValue == null || uninstallValue == "") {
							error("Check condition '"
								+ checkCond
								+ "' cannot be executed"
								+ " since no version information is available for '"
								+ checkPath + "'" + " at "
								+ uninstallLocations[iUninstKey] + ".");
							...
							var uninstallVersionCompare = versionCompare(
								uninstallValue, checkValueExpanded);
							dinfo("Comparing uninstall version '" + uninstallValue
								+ "' to expected version '"
								+ checkValueExpanded + "' using condition '"
								+ checkCond + "' returned "
								+ uninstallVersionCompare + ".");
							...
							switch (checkCond) {
								...
								case "versiongreaterthan":
									...
									break;
								default:
									error("Unknown operation on uninstall version check: "
										+ checkCond + ".");
									uninstallVersionCompResult = false;
									break;
							}

							dinfo("Uninstall version check for package '"
								+ checkPath + "' returned "
								+ uninstallVersionCompResult
								+ " for operation type " + checkCond + ".");

							// in case the current entry does not match the
							// condition,
							// immediately return
		...
		case "logical":
			...
			switch (checkCond) {
				...
				case "atleast":
					if (checkValue == null) {
						throw new Error(
							"Check condition logical 'atleast' requires a value.");
					}
 ...
 *
 * @param attributeName
 *            Name of host attribute to match. See getHostInformation() function
 *            for valid host attributes.
 * @param expression
...
function checkHostAttribute(attributeName, expression) {
	...
	if (expression == null) {
		error("Host attribute matching for attribute '" + hostAttribute
			+ "' failed. No expression specified.");
		return false;
	}
	// Expand environment variables in expressions.
	var checkExpression = new ActiveXObject("WScript.Shell")
		.ExpandEnvironmentStrings(expression);
	...
	if (hostInfoValue == null
		|| (typeof (hostInfoValue) == "object" && hostInfoValue.length <= 0)) {
	...
	if (typeof (hostInfoValue) == "object" && hostInfoValue.length > 0) {
		for (var iHostInfo = 0; iHostInfo < hostInfoValue.length; iHostInfo++) {
			...
			dinfo("Comparing multi-valued attribute '" + hostAttribute
				+ "' with value '" + hostInfoElement
				+ "' using expression '" + checkExpression + "'.");

			// Compare attribute array element with expected
			// value.
			if (attrMatchExpression.test(hostInfoElement) == true) {
				dinfo("Match for attribute '" + hostAttribute
					+ "' with value '" + hostInfoElement + "' found.");
				returnValue = true;
				break;
			}
		}
		...
		switch (hostAttribute) {
			case "environment":
				...
				// Get condition value from from parameter, could be multiple,
				// separated by '|'.
				...
				for (var iEnv = 0; iEnv < environmentConditions.length; iEnv++) {
					...
					// Need at least the key and value. If there are less
					// components, then skip it.
					if (envConditionSplit.length >= 2) {
						...
						var envValueRead = new ActiveXObject("WScript.Shell")
							.ExpandEnvironmentStrings(expandString);

						if (envValueRead == expandString) {
							// Environment variable is not defined.
							dinfo("Environment variable " + envKey
								+ " is not defined, comparing to empty string.");
							envValueRead = "";
						}

						// All following values are belonging to the value.
						/*
						 * var valueParts = new Array(); for (var iValues=1; iValues <
						 * envConditionSplit.length; iValues++) {
						 * valueParts.push(envConditionSplit[iValues]); } // Join
						 * values to re-assemble the value specified. var envValue =
						 * valueParts.join("");
						 */

						// Re-assemble value.
						var valueStartOffset = envKey.length + 1;
						var envValue = environmentCondition
							.substr(valueStartOffset);
						...
						if (envMatchExpression.test(envValueRead) == true) {
							dinfo("Required environment matched. Environment variable '"
								+ envKey
								+ "' with value '"
								+ envValueRead
								+ "' matches '" + envValue + "'.");
							...
							dinfo("Required environment did not match. Environment variable '"
								+ envKey
								+ "' with value '"
								+ envValueRead
								+ "' does not match '" + envValue + "'.");
							returnValue = false;
							break;
						}
					} else {
						error("Invalid environment match expression '"
							+ environmentCondition + "'. Match skipped.");
					}
				}
				break;

			case "lcid":
				// Check whether any LCID matches the current host executing user
				// LCID.
				...
				for (var iLCID = 0; iLCID < attributeLCIDs.length; iLCID++) {
					...
					if (currentLcid == hostInfoValue) {
						dinfo("Required LCID match found. LCID '" + currentLcid
							+ "' matches current user LCID.");
						returnValue = true;
						break;
					}
				}
				if (!returnValue) {
					dinfo("None of the required LCID values (" + checkExpression
						+ ") matched the current host LCID of '"
						+ hostInfoValue + "'.");
				}
				break;
			...
			case "lcidOS":
				...
				for (var iLCIDOS = 0; iLCIDOS < attributeLCIDs.length; iLCIDOS++) {
					...
					if (currentLcid == hostInfoValue) {
						dinfo("Required OS LCID match found. LCID '" + currentLcid
							+ "' matches current host LCID.");
						returnValue = true;
						break;
					}
				}
			...
			default:
				...
				if (attrMatchExpression.test(hostInfoValue) == true) {
					dinfo("Host attribute '" + hostAttribute + "' with value '"
						+ hostInfoValue + "' matches expression '"
						+ checkExpression + "'.");
					returnValue = true;
				} else {
					dinfo("Host attribute '" + hostAttribute + "' with value '"
						+ hostInfoValue + "' does not match expression '"
						+ checkExpression + "'.");
					returnValue = false;
				}
				break;
 ...
 *
 * @param fileName
 *            String pointing to the settings file to be created (full path).
...
function downloadClean(downloadNode) {
	...
	target = new ActiveXObject("WScript.Shell")
		.ExpandEnvironmentStrings(target);
	var fso = new ActiveXObject("Scripting.FileSystemObject");
 ...
 * @return command defined within the given cmd XML node, returns null if no
 *         command is defined.
 */
...
/**
 * Return value of include attribute of the given cmd node. Returns null if
 * include attribute is not set.
 *
 * @param cmdNode
 ...
 *
 * @param xmlNode
 *            XML node which is supposed to have a <condition /> sub-node.
 * @returns Array of condition XML-nodes, might be null if no condition is
 *          specified
 */
function getConditions(xmlNode) {
	...
	/*
	 * var conditionNodes = xmlNode.selectNodes("wpkg:condition"); if
	 * (conditionNodes.length <= 0) { // Maybe namespace has not been specified
	 * correctly. // Try reading from default namespace. conditionNodes =
	 * xmlNode.selectNodes("condition"); }
	 */

	// Per specification only one single condition node shall be specified
	/*
	 * if (conditionNodes != null && conditionNodes.length > 1) { error("More
	 * than one condition node specified. Ignoring all but the first
	 * condition."); }
	 */
 ...
 * @param downloadNode
 *            download XML node
 * @returns {Number} Value of 'timeout' attribute, returns value of
 *          downloadTimeout if no timeout value exists or it cannot be parsed.
 *          Returns integer.
 */
 ...
 *
 * @param downloadNode
 *            The download XML node.
 * @returns true if variables shall be expanded in URL attribute, false if they
 *          should not be expanded. Defaults to true if attribute is undefined.
 */
...
function getFileSize(file) {
	...
	try {
		...
		var expandedPath = new ActiveXObject("WScript.Shell")
		...
		size = -1;
		dinfo("Unable to get file size for '" + file + "': " + e.description);
	}
 ...
 * @returns Date when the file has been created. Returns null if file date could
 *          not be read.
 */
 ...
 * @returns Date when the file has been last modified. Returns null if file date
 *          could not be read.
 */
 ...
 * @returns Date when the file has been last accessed. Returns null if file date
 *          could not be read.
 */
...
function getFileVersion(file) {
	...
	try {
		...
		version = null;
		dinfo("Unable to find file version for " + file + " : " + e.description);
	}
	return version;
}
		...
		try {
			...
			for (; !osEnum.atEnd(); osEnum.moveNext()) {
				...
				if (osItem.CSDVersion != null) {
					CSDVersion = osItem.CSDVersion.replace(/Service Pack /i,
						"SP");
				}
				var strSystem = trim(osItem.Caption) + ", "
					+ OtherTypeDescription + ", " + CSDVersion + ", "
					+ osItem.Version;
...
function getDomainName() {
	if (domainName == null) {
		try {
			...
			var QueryRes = objWMIService
				.ExecQuery("Select * from Win32_ComputerSystem where PartOfDomain=True ");
...
function getHostGroups() {
	if (hostGroups == null) {
		...
		try {
			...
			var obj = GetObject("WinNT://" + domainName + "/" + hostName
				+ "$,user");
			var groups = obj.Groups();
			for (var item = new Enumerator(groups); !item.atEnd(); item
				.moveNext()) {
				var group = item.item();
...
/**
 * Returns a list of attribute/value pair associated to the host definition in
 * hosts.xml.
 *
 * @param hostNode
	...
	if (hostNode.attributes != null) {
		for (var i = 0; i < hostNode.attributes.length; i++) {
			if (hostNode.attributes[i].value != null) {
				hostAttributes.Add(hostNode.attributes[i].name,
					hostNode.attributes[i].value);
			}
		}
	}
...
function getHostInformation() {
	...
	if (hostAttributes == null) {
		...
		dinfo("Host properties: " + "hostname='"
			+ hostAttributes.Item("hostname") + "'\n" + "architecture='"
			+ hostAttributes.Item("architecture") + "'\n" + "os='"
			+ hostAttributes.Item("os") + "'\n" + "ipaddresses='"
			+ hostAttributes.Item("ipaddresses").join(",") + "'\n"
			+ "domain name='" + hostAttributes.Item("domainname") + "'\n"
			+ "groups='" + hostAttributes.Item("groups").join(",") + "'\n"
			+ "lcid='" + hostAttributes.Item("lcid") + "'\n" + "lcidOS='"
			+ hostAttributes.Item("lcidOS") + "'");
	}
	return hostAttributes;
}
	...
	for (var i = 0; i < xmlNodes.length; i++) {
		...
		for (var iAttribute = 0; iAttribute < xmlNode.attributes.length; iAttribute++) {
			if (hostInformation.Item(xmlNode.attributes[iAttribute].name) != null) {
				xmlNodeAttrs.Add(xmlNode.attributes[iAttribute].name,
					xmlNode.attributes[iAttribute].value);
			}
		}
		...
		for (var iAttr = 0; iAttr < attrsKeys.length; iAttr++) {
			...
			var attributeMatchFound = checkHostAttribute(xmlNodeAttrName,
				xmlNodeAttrValue);

			// Verify if the attribute does match to current host.
			if (attributeMatchFound != true) {
				// No match found. Advance to next host.
				dinfo("No value of '" + xmlNodeAttrName + "' matched '"
					+ xmlNodeAttrValue + "'. Skipping to next definition.");
				hostMatchFound = false;
				break;
			}
			/*
			 * else { // This attribute matched, continue with next attribute
			 * hostMatchFound = true; continue; }
			 */
		}

		// If not all attributes match the current host definition then the node
		// is not included.
		// All nodes which do not specify advanced host match attributes are
		// included too.
		if (hostMatchFound) {
			// All attributes matched

			// Print some debug information about which extended host attributes
			// matched.
			if (xmlNodeAttrs.count > 0) {
				var attrsKeys = xmlNodeAttrs.keys().toArray();
				var attrDesc = new Array();
				for (var iAttrKeys = 0; iAttrKeys < attrsKeys.length; iAttrKeys++) {
					attrDesc.push(attrsKeys[iAttrKeys] + "="
						+ xmlNodeAttrs.Item(attrsKeys[iAttrKeys]));
				}
				dinfo("XML node with special host attribute match found: "
					+ attrDesc.join(", "));
			}
 ...
 *         host XML nodes if applymultiple is true. Returns array with zero
 *         elements if no host node matches.
 */
	...
	if (applyingHostNodes == null) {
		...
		for (var iHost = 0; iHost < hostNodes.length; iHost++) {
			...
			if (hostNameAttribute != null && hostNameAttribute != "") {
				...
				if (hostNameAttribute.toUpperCase() == getHostname()
					.toUpperCase()) {
					...
					try {
						...
						for (var iIPAdresses = 0; iIPAdresses < ipAddresses.length; iIPAdresses++) {
							...
							if (splitIP.length == 4 && splitPattern.length == 4) {
								var firstValue = 0;
								var secondValue = 0;
								var match = true;
								for (var k = 0; k < splitIP.length; k++) {
									...
									// let's finally see if the ip octet is
									// outside the range
									if ((ipOctet < firstValue || ipOctet > secondValue)) {
										// if octet did not match the
										// requirements
										// dinfo("no match!");
										match = false;
										// no need to continue
					...
					// If we still got no match with, then try regular
					// expression matching.
					if (!ipMatchSuccess) {
						try {
							var hostNameAttributeMatcher = new RegExp("^"
								+ hostNameAttribute + "$", "i");
				...
				// Host "name" attribute is missing or empty. Include host as
				// potential match.
				// This allows to filter this host later using extended host
				// matching
				hostNodesApplying.push(hostNode);
			}
		...
		// hostNodesApplying = filterConditionalNodes(hostNodesApplying,
		// isApplyMultiple());
		...
		// It's not an error if no host nodes have been found. Empty array is
		// allowed as well.
...
/**
 * Returns the profile-id associated with the given host node. The node
 ...
 * @param hostNode
 *            XML node of the host definition
 * @return array of strings with referenced profiles (array might be of length 0
 *         if no profiles are defined)
 */
 ...
 *
 * @param array
 *            Object of type Array to which the the variables appended. In case
 *            null is supplied it returns a new Array object.
...
function getHostsVariables(array) {
	...
	if (hostsVariables == null) {
		...
		for (var iHostNode = 0; iHostNode < hostNodes.length; iHostNode++) {
			...
			dinfo("Reading variables from host: "
				+ getHostNodeDescription(hostNode));
	...
	if (array != null) {
		// concatenatedVariables = concatenateDictionary(dictionary,
		// hostsVariables);
	...
	if (languageNode == null && getConfig() != null) {
		...
		if (languagesNodes != null) {
			...
			for (var i = 0; i < languagesNodes.length; i++) {
				...
				for (var j = 0; j < languageNodes.length
					&& languageNode == null; j++) {
					...
					for (var k = 0; k < lcids.length; k++) {
						...
						if (currentLcid == getLocale()) {
							dinfo("Found language definition node for language ID "
								+ currentLcid);
 ...
 *
 * "always" (default): When a package is new to the host then first the checks
 * are run in order to verify whether the package is already installed. If the
 * checks succeed then it is assumed that no further installation is needed. The
 * package is silently added to the host without executing any commands.
 *
 * "never": When a package is new to the host then the install commands are run
 * in any case (without doing checks first). Note: Checks will still be done
 * after package installation to verify whether installation was successful.
 *
 * @param packageNode
 *            Package XML node to read attribute from.
 ...
 * Currently supported values:
 *
 * "always": When a package is removed from a host then the checks will be
 * executed before removal is processes. If the checks fail this potentially
 * means that the package has been removed already. In such case the package
 * remove commands will be skipped.
 *
 * "never" (default): When a package is about to be removed from the host then
 * WPKG will execute the remove commands in any case without executing the
 * checks first. Note: Checks will still be done after package removal to verify
 ...
 *
 * "never" (default): When a package is about to be upgraded then WPKG will
 * execute the upgrade commands in any case without executing the checks first.
 * This is the recommended behavior. Note: Checks will still be done after
 * package upgrade to verify whether the upgrade was successful.
 *
 * @param packageNode
 *            Package XML node to read attribute from.
 ...
 * Currently supported values:
 *
 * "always": When a package is downgraded the checks specified will be be
 * executed before the downgrade takes place. If checks succeed, then the
 * downgrade will not be performed (WPKG just assumes that the old version is
 * already applied correctly. Please note that your checks shall verify a
 * specific software version and not just a generic check which is true for all
 * versions. If your checks are true for the new/current version too then WPKG
 * would never perform the downgrade in this mode.
 *
 * "never" (default): When a package is about to be downgraded then WPKG will
 * execute the downgrade commands in any case without executing the checks
 * first. This is the recommended behavior. Note: Checks will still be done
 * after package downgrade to verify whether the downgrade was successful.
 *
 * @param packageNode
 *            Package XML node to read attribute from.
...
/**
 * Returns an array of <check /> XML sub-nodes on a given XML node. In case
 * extended host matching attributes are used only the checks which match the
 * current host are returned.
 *
 * @param xmlNode
 *            The XML node from which all 'check' sub-nodes are read
 * @return Array of XML nodes containing all 'check'-nodes which match to the
 *         current host. Returns empty array if no checks are defined. If
 *         extended host matching attributes like "hostname", "os" or similar
 *         are used then checks which do not match the current host are not
 *         returned.
 */
function getChecks(xmlNode) {
	var checkNodes = xmlNode.selectNodes("check");
	/*
	 * var checkNodes = xmlNode.selectNodes("wpkg:check"); if (checkNodes.length <=
	 * 0) { // Maybe namespace was wrongly specified. // Try default namespace.
...
/**
 * Returns a list of commands which apply to the given command type. Common
 * types are 'install', 'upgrade', 'downgrade' or 'remove' but WPKG allows any
 * custom type definition within the commands/command XML structure. For example
 * it is possible to specify <command type="test-type" /> and then receive all
 * "test-type" commands using this method.
 *
 * @param packageNode
 ...
 *            included. This is used to detect inclusion loops (recursive
 *            inclusion).
...
function getPackageCmd(packageNode, type, includeChain) {
	...
	// Fetch command-nodes from <commands><command type="type" /></commands>
	// structure.
	var commandNodes = packageNode.selectNodes("commands/command[@type=\""
		+ type + "\"]");
	...
	for (var iTypeCommands = 0; iTypeCommands < commandNodeList.length; iTypeCommands++) {
		...
		if (include != null) {
			...
			var includedCommands = getPackageCmd(packageNode, include,
				prevIncluded);
	...
	if (packageNode != null) {
		...
		if (dependNodes != null) {
			...
			for (var i = 0; i < matchingDependNodes.length; i++) {
				var dependId = matchingDependNodes[i]
					.getAttribute("package-id");
...
function getPackageIncludes(packageNode, packageList) {
	...
	if (packageNode != null) {
		...
		if (includeNodes != null) {
			var matchingIncludeNodes = filterConditionalNodes(includeNodes,
				true);
			for (var i = 0; i < matchingIncludeNodes.length; i++) {
				var dependId = matchingIncludeNodes[i]
					.getAttribute("package-id");
...
/**
 * Reads the "manualInstall" attribute from a package node. This attribute is
 * true only if the package as installed manually via command line. It is false
 * for packages which are initially installed by package synchronization.
 *
 * @param packageNode
 *            the package from which the attribute is read.
 ...
 * the local settings database. If even the local database does not contain such
 * a package entry then it prints an error about missing package definition. In
 * case '/quitonerror' is set it exits.
 *
 * If the package could be located within the local package database it prints a
 * warning and returns the local package node.
 *
...
function getPackageNodeFromAnywhere(packageID) {
	...
	if (packageDBNode != null) {
		...
		/*
		 * var packageNotFoundMessage = "Profile inconsistency: Package '" +
		 * packageID + "' does not exist within the package database. " +
...
/**
 * Returns the package reboot attribute value. This attribute can add additional
 * reboots but not limit or invalidate reboot flags set on the command-level.
 *
 ...
 * </pre>
 *
 * This means that an immediate reboot always has the highest priority. You can
 * just set "reboot markers" on a "timeline" on package and command level where
 * the closest reboot marker will be executed: immediate => package => postponed =>
 * none
 *
 * @return one of the states (string values): "true", always reboot after
 *         package installation "postponed", reboot before script exits "false",
 *         reboot only if command specified reboot=delayed/postponed
 *
 */
	...
	for (var i = 0; i < linkedPackageIDs.length; i++) {
		...
		if (currentNode != null) {
			if (!searchArray(packageArray, currentNode)) {
				dinfo("Adding referenced package '"
					+ getPackageName(currentNode) + "' ("
				...
				dinfo("Referenced package '" + getPackageName(currentNode)
					+ "' (" + getPackageID(currentNode) + ") for package '"
					+ getPackageName(packageNode) + "' ("
					+ getPackageID(packageNode) + ") already added.");
			}
		}
	}
...
function getPackageRevision(packageNode) {
	...
	if (packageRevision == null) {
		...
		if (packageRevision.match(new RegExp("%.+%"), "ig")) {
			...
			var wshObject = new ActiveXObject("WScript.Shell");
			packageRevision = wshObject
				.ExpandEnvironmentStrings(packageRevision);
 ...
 * applied to the current host. Valid actions are: "none" No action; package
 * installed already "install" Installation, package is new on the host
 * "upgrade" Upgrade package which already exists on the system New version
 * higher than installed version "downgrade" Downgrade package which already
 * exists on the system New version lower than installed version
 *
 * @param packageNode
 *            The package to be checked.
 * @returns Action to be performed. Can be 0=nothing, 1=install, 2=upgrade,
 *          3=downgrade.
 */
function getPackageInstallAction(packageNode) {
	...
	if (installedPackage == null) {
		...
		dinfo(packageMessage
			+ "Not in local package database; Marking for installation.");
		...
		if (comparisonResult > 0) {
			...
			info(packageMessage + "Already installed but version mismatch.\n"
			...
			if (executeAttr == "always") {
				...
				// Packages with exec attribute "always" will be installed on
				// each run; regardless of their version.
				dinfo(packageMessage
					+ "Is requested to be executed 'always'. Preparing installation.");
				action = actionInstall;

			} else if (isForceInstall()) {
				// if installation is forced, install anyway
				info(packageMessage
					+ "Already installed. Re-installation enforced.");
				action = actionInstall;
				...
				if (executeAttr == "once") {
					...
					if (getQueryMode() == "remote") {
						...
						if (isInstalled(installedPackage)) {
							...
							// Package found in local database but checks
							// failed.
...
function getPackagesManuallyInstalled() {
	if (manuallyInstalled == null) {
		...
		manuallyInstalled = settings
			.selectNodes("package[@manualInstall=\"true\"]");
...
/**
 * Returns an array of packages which are not assigned to the current host any
 * more.
 *
 * Packages which are manually installed are not included in the list of
 * packages to be removed. Except if the package does not exist on server side
 * any more. Therefore in case a package is removed from the server it is
 * removed from clients as well even if the package was installed manually
 * because it is to be assumed tha the administrator no longer wants to support
...
function getPackagesRemoved() {
	...
	for (var iInstalledPkg = 0; iInstalledPkg < installedPackages.length; iInstalledPkg++) {
		...
		dinfo("Found installed package '"
			+ getPackageName(installedPackageNode) + "' ("
		...
		// If package is no longer present, mark for remove if not installed
		// manually.
		if (!found) {
			// Check if package was installed manually.
			// Manually installed packages remain on the system.
			var packageMessage = "Package '"
				+ getPackageName(installedPackageNode) + "' ("
				+ getPackageID(installedPackageNode) + "): ";
			...
			for (var iLocalPackage = 0; iLocalPackage < localPackagesAndDependencies.length; iLocalPackage++) {
				...
				if (getPackageID(installedPackageNode) == getPackageID(localPackage)) {
					// Package is either locally installed or within the
					// dependency tree of
...
function getProfileDependencies(profileNode) {
	...
	if (dependNodes != null) {
		...
		var matchingDependNodes = filterConditionalNodes(dependNodes, true);
		for (var i = 0; i < matchingDependNodes.length; i++) {
			var dependencyId = matchingDependNodes[i]
...
function getProfileList() {
	if (applyingProfilesDirect == null) {
		...
		if (argn("profile") != null) {
			...
			var hostNodes = getHostsApplying();
			for (var ihostNode = 0; ihostNode < hostNodes.length; ihostNode++) {
				profilesMatching = profilesMatching
					.concat(getHostProfiles(hostNodes[ihostNode]));
			}
			if (profilesMatching.length <= 0) {
				throw new Error("Could not find any profile for host "
					+ getHostname() + ".");
			}
		}
 ...
 * packages as referred in profiles.xml. Dependency information is only
	...
	for (var i = 0; i < profileArray.length; i++) {
		...
		for (var j = 0; j < packageNodes.length; j++) {
			...
			if (installDate != null || uninstallDate != null) {
				...
				if (now >= installDate && now <= uninstallDate) {
					includePackage = true;
					dinfo("Package'" + packageId
						+ "' specified an install date range: "
						+ installDate + " to " + uninstallDate
						+ "; current time (" + now
						+ ") is within the time frame. Including package.");
				} else {
					includePackage = false;
					dinfo("Package '" + packageId
						+ "' specified an install date range: "
						+ installDate + " to " + uninstallDate
						+ "; out of range, skipping package (local time: "
						+ now + ").");
				}
			}
 ...
 *            definition
 * @return date object representing installation date. Null if date is
 *         undefined.
 */
 ...
 *
 * NOTE: Since the profile just contains the package IDs referenced within
 * profiles.xml but not existing within the packages database (packages.xml)
...
function getProfilePackageNodes() {
	if (profilePackageNodes == null) {
		...
		for (var i = 0; i < packageIDs.length; i++) {
			...
			dinfo("Adding package with ID '" + packageID
				+ "' to profile packages.");
			...
			if (packageNode != null) {
				...
				if (!searchArray(packageNodes, packageNode)) {
					// Add the new node to the array _after_ adding
					// dependencies.
	...
	if (applyingProfilesAll == null) {
		...
		for (var i = 0; i < profileList.length; i++) {
			...
			if (profileNode != null) {
				...
				error("Profile '" + profileList[i]
					+ "' applies to this host but was not found!");
			}
		}
 ...
 *
 * @param array
 *            Object of type Array to which the the variables are appended. In
 *            case null is supplied it returns a new Array object.
 * @return Object of type Scripting.Dictionary which contains all key/value
...
function getProfileVariables(array) {
	...
	if (profilesVariables == null) {
		...
		/*
		 * add each profile's variables to the array in reverse order reversing
		 * the order is done in order to allow overwriting of variables from
		 * dependent profiles
		 */
...
/**
 * Tries to read host attributes from the settings database. All host attributes
 * found in the settings database will be used to override attributes of the
 * local host.
 */
 ...
 *
 * @param checkNode
 *            Check XML node to be inserted.
 * @param result
 *            Result of the check on current node.
 */
function addSettingsCheckResult(checkNode, result) {
	try {
		...
		// If a check was found then remove it from the results in order to
		// avoid
		// duplicate entries. Checks might also be executed multiple times (with
		// potentially different results) and only the last result should be
		// kept.
 ...
 *
 * @param checkNode
 *            the check node for which to look in the settings "checkResults"
 *            nodes to verify if the check has been executed already.
 *
 * @returns result of already evaluated check. Returns null if the check has not
 *          been evaluated and saved to settings node before.
 */
 ...
 *
 * @param checkNode
 *            check to seek for in local settings databse.
 *
...
function getSettingsCheck(checkNode) {
	...
	if (checkResults != null) {
		...
		if (attributes.length > 0) {
			...
			dinfo("Searching for previously executed checks with attributes "
				+ checkMessage);
			...
			if (checkNodes != null) {
				...
				for (var iCheck = 0; iCheck < checkNodes.length; iCheck++) {
					...
					// dinfo("Found previously executed check: " +
					// checkNode.xml);
 ...
 * @param array
 *            Object of type Array to which the the variables are appended. In
 *            case null is supplied it returns a new Array object. Each array
 *            element is a dictionary object containing a key and a value. The
 *            key is the variable name and the value is the variable value to be
 *            assigned.
	...
	for (var i = 0; i < variableNodes.length; i++) {
		...
		if (variableName == null || variableValue == null) {
			error("Incomplete variable specification found. "
				+ "Variable name is '" + variableName
				+ "' and variable value is '" + variableValue
				+ "'. Ignoring variable.");
			continue;
		}
...
function installPackage(packageNode) {
	...
	dinfo("Going to install package '" + packageName + "' (" + packageID
		+ "), Revision " + packageRev + ", (execute flag is '"
		+ executeAttr + "', notify flag is '" + notifyAttr + "').");
	...
	if (searchArray(packagesInstalled, packageNode)) {
		...
		dinfo(packageMessage + "Already installed once during this session.\n"
			+ "Checking if package is properly installed.");
		bypass = true;

		// check if installation of package node was successful
		if ((installedPackage != null)
			&& (versionCompare(getPackageRevision(installedPackage),
				packageRev) >= 0)) {
			...
			dinfo(packageMessage + "Installation failed during this session.");
		...
		switch (packageAction) {
			...
			case "install":
				...
				// If execute attribute is set to "always" just continue with
				// installation.
				if (executeAttr == "always") {
					break;
				}
				if (installCheckPolicy == "never") {
					// Checks shall be bypassed and package is installed in any
					// case.
					dinfo(packageMessage
						+ "Skipping checks whether package is already installed.");
				} else {
					...
					if (isInstalled(packageNode)) {
						...
						var depSuccess = installPackageReferences(packageNode,
							"dependencies");
						if (depSuccess) {
							info(packageMessage
						...
						var chainedSuccess = installPackageReferences(packageNode,
							"chained");
						if (chainedSuccess) {
							info(packageMessage
						...
						// Bypass installation as installations seems to be done
						// already.
						bypass = true;
						installType = typeInstall;
	...
	if (!bypass) {
		...
		try {
			...
			var depInstallSuccess = installPackageReferences(packageNode,
				"dependencies");
			...
			logStatus("Performing operation (" + installType + ") on '"
			...
			if (getChecks(packageNode).length > 0 && !isInstalled(packageNode)) {
				...
				if (rebootRequired || rebootAttr == "true") {
					...
					var failMessage = "Could not process (" + installType
						+ ") " + packageName + ".\n"
				...
				var chainedStatus = installPackageReferences(packageNode,
					"chained");
				if (chainedStatus) {
					info(packageMessage
				...
				if (rebootRequired || rebootAttr == "true") {
					info("Installation of " + packageName
						+ " successful, system " + "rebooting.");
					reboot();
				} else if (rebootPostponed || rebootAttr == "postponed") {
					info("Installation of " + packageName
						+ " successful, postponed reboot scheduled.");
					setPostponedReboot(true);
				} else {
					info("Processing (" + installType + ") of " + packageName
						+ " successful.");
				}
			}
		} catch (err) {
			success = false;
			var errorMessage = "Could not process (" + installType
			...
			if (lnkDir != null) {
				if (category != null) {
					info("Deplacement automatique des raccourcis de id="
						+ packageID + " depuis " + startmenu + "\\"
						+ lnkDir + " dans la Category " + category + ".");
					exec("%ComSpec% /C call %Z%\\wpkg\\AnalyseCategory.bat \""
						+ packageID + "\" \"" + lnkDir + "\"", timeout,
						workdir);
				} else {
					info("Pas de deplacement automatique des raccourcis car 'category' est absent dans "
						+ packageID + ".");
				}
			} else {
				info("Pas de deplacement automatique des raccourcis car 'lnk' est absent dans "
					+ packageID + ".");
			}
...
/**
 * Installs all packages references of the selected type. Returns true in case
 * all references could be installed. Returns false if at least one reference
 * failed.
 *
 * @param packageNode
 *            package to install the references of (XML node) NOTE: The package
 *            itself is not installed.
 * @param referenceType
 *            select "dependencies" or "chained". Defaults to "dependencies".
...
function installPackageReferences(packageNode, referenceType) {
	...
	for (var i = 0; i < references.length; i++) {
		...
		if (refPackage == null) {
			...
			if (!success) {
				problemDesc += "Installation of reference (" + type
					+ ") package '" + getPackageName(refPackage) + "' ("
 ...
 *
 * @param name
 *            Package ID of package to be installed.
 * @param manualInstall
 *            Boolean value specifying whether the package is manually added.
 *            These packages are handled differently and not removed during
 *            synchronization.
 */
 ...
 * in order to be able to access 64-bit directories and registry keys. The
 * 64-bit instance of cscript is located at %SystemRoot%\system32\. If cscript
 * from %SystemRoot%\SysWOW64\ is used (32-bit binary) then all reads to
 ...
 *
 * @return true in case downloads shall be disabled, false if downloads are
 *         enabled
 */
...
function isZombie(packageNode) {
	...
	if (zombie) {
		var errorMessage = "Error while synchronizing package "
			+ packageName
...
/**
 * Query and print host information read from settings file. This requires that
 * host information is available in settings file. You must have
 ...
 * @param packageAction
 *            Optional argument to include the action applied to the package in
 *            the package information. If set to null the package action
		...
		if (settingNode != null && packageAction != null) {
			...
			if (newPackageRevision != oldPackageRevision) {
				message += "    Revision (new):    "
					+ getPackageRevision(packageNode) + "\n";
				message += "    Revision (old):    "
					+ getPackageRevision(settingNode) + "\n";
			} else {
				message += "    Revision:          "
					+ getPackageRevision(packageNode) + "\n";
			}
 ...
 *
 * @param listInstall
 *            List packages which are pending to be installed.
 * @param listUpgrade
 *            List packages which are pending to be upgraded.
 * @param listDowngrade
 *            List packages which are pending to be downgraded.
 * @param listRemove
 *            List packages which are pending to be removed.
 * @param listUnmodified
 *            List packages which are not modified during synchronization.
 */
function queryProfilePackages(listInstall, listUpgrade, listDowngrade,
	listRemove, listUnmodified) {
	// Message to be shown as a result of query.
	var message = "Current profile packages:\n";

	// Message which is appended when system is modified (includes
	// execute="change" packages).
	...
	for (var i = 0; i < profilePackageNodes.length; i++) {
		...
		switch (packageAction) {
			...
			case "install":
				if (listInstall) {
					packageMessage += queryPackage(packageNode,
...
function removePackage(packageNode) {
	...
	if (searchArray(packagesRemoved, packageNode)) {
		...
		dinfo(packageMessage + "Already removed once during this session.\n"
	...
	if (checkPolicy == "always" && !isInstalled(packageNode)) {
		dinfo(packageMesseage
			+ "Package already removed from system. Skipping removal.");
			...
			if (!allSuccess && !isForce()) {
				...
				try {
					...
					for (var iCommand = 0; iCommand < cmds.length; iCommand++) {
						...
						var exitAction = getCommandExitCodeAction(cmdNode,
							result);
						...
						if (exitAction != null) {
							if (exitAction == "reboot") {
								...
								info("Command in removal of " + packageName
									+ " returned " + "exit code [" + result
									+ "]. This exit code "
									+ "requires an immediate reboot.");

								// Verify if the package is a zombie (not in
								// package
								// database any more). If it is a zombie, and
								// not referenced
								// in the profile then prevent endless reboots
								// by removing
								...
								if (isZombie(packageNode)) {
									...
									// within the profile remove the settings
									// entry
					...
					if (lnkDir != null) {
						if (category != null) {
							info("Suppression des raccourcis de id="
								+ packageID + " depuis " + startmenu + "\\"
								+ category + "\\" + lnkDir);
							exec(
								"%ComSpec% /C call %Z%\\wpkg\\AnalyseCategory.bat \""
								+ packageID + "\" \"" + lnkDir
								+ "\" remove", timeout, workdir);
						} else {
							info("Pas de suppression automatique des raccourcis car 'category' est absent dans "
								+ packageID + ".");
						}
					} else {
						info("Pas de suppression automatique des raccourcis car 'lnk' est absent dans "
							+ packageID + ".");
					}
					// FIN PATCH SE4
					// restore old environment
			...
			if (!isInstalled(packageNode)) {
				...
				if (isZombie(packageNode)) {
					...
					if (!referenced && !isNoForcedRemove()) {
						removeSettingsNode(packageNode, true);
						warning("Errors occurred while removing '"
							+ packageName
							+ "' ("
							+ packageID
...
function removePackagesDependent(packageNode) {
	...
	if (depSuccess) {
		dinfo("Removal of depending packages for '" + packageName + "' ("
			+ packageID + ") successfully finished.");
	} else {
		var failMessage = "Removal of depending packages for '" + packageName
 ...
 *            Architecture to used for this host.
 */
 ...
 *
 * @param newState
 *            new value for the no-download flag (boolean). If set to true then
 *            all downloads are disabled (just skipped).
 */
...
/**
 * Set a new value for the manual installation flag of the given package. Manual
 * installations are flagged only for packages which are installed via command
 * line directly and not via synchronization.
 *
 * @param packageNode
 *            package to be modified.
 * @param manualInstall
 *            {Boolean} new value of package installation flag.
 */
...
/**
 * Set path to local settings file (local package database). The path might
 * contain environment variables as well as the following expressions:
 * [HOSTNAME] Replaced by the executing hostname. [PROFILE] Replaced by the
 * concatenated list of profiles applied.
 *
 * @param path
 ...
 *            "NAME"
 * @param sortOrder
 *            order in which the elements are sorted (integer) valid values:<br>
 *            1 sort ascending (default)<br>
 *            2 sort descending
 *
...
function synchronizeProfile() {
	 ...
	 */
	var profilePackageNodes = getProfilePackageNodes();
	dinfo("Synchronizing. Number of packages referenced by profile: "
		+ profilePackageNodes.length + ".");

	var localPackages = getPackagesManuallyInstalled();
	if (localPackages.length > 0) {
		dinfo("Synchronizing. Locally installed packages: "
	 ...
	 *
	 * This was done to allow fixing a broken uninstall-procedure on server
	 * side. Without upgrading to the latest version here it might happen that
	 * the package cannot be removed without the possibility to fix it. If you
	 * remove the package completely from the package database it will be forced
	 * to be removed from the local settings file even if uninstall fails.
	 *
	 * NOTE: This is not done within the same loop as the removal (see below) in
	 * order to prevent re-installing already removed dependencies.
	 */
	...
	if (isUpgradeBeforeRemove()) {
		...
		for (var iSortedPkg = 0; iSortedPkg < sortedUpgradeList.length; iSortedPkg++) {
			...
			if (serverPackage != null) {
				logStatus("Remove: Checking status of '"
					+ getPackageName(serverPackage) + "' ("
	...
	for (var iInstallPkg = 0; iInstallPkg < packagesToInstall.length; iInstallPkg++) {
		...
		logStatus("Install: Verifying package '"
			+ getPackageName(packagesToInstall[iInstallPkg]) + "' ("
	...
	for (var iChangeAwait = 0; iChangeAwait < packagesAwaitingChange.length; iChangeAwait++) {
		...
		if (isSystemChanged()) {
			logStatus("Install: Verifying package (system changed) '"
				+ getPackageName(packagesAwaitingChange[iChangeAwait])
				+ "' (" + (packagesToInstall.length + iChangeAwait + 1)
				+ "/" + sortedPackages.length + ")");

			installPackage(packagesAwaitingChange[iChangeAwait]);
		} else {
			logStatus("Install: No system change, skipping '"
				+ getPackageName(packagesAwaitingChange[iChangeAwait])
				+ "' (" + (packagesToInstall.length + iChangeAwait + 1)
				+ "/" + sortedPackages.length + ")");
		}
...
function saveXml(root, path) {
	...
	var xmlDoc = new ActiveXObject("Msxml2.DOMDocument.3.0");
	var processing = xmlDoc.createProcessingInstruction("xml",
 ...
 * @param rootNS
 *            Optionally specify a namespace. e.g.
 ...
 *            into generated XML document. Supported types: - settings (local
 *            WPKG database XML) - hosts (hosts database) - profiles (profile
 *            database) - packages (package database) - config (configuration
 *            file)
...
function loadXml(xmlPaths, type) {
	...
	// Name of child elements to be read if multiple files are read from
	// directory.
	...
	// source.setProperty("SelectionNamespaces",
		...
		if (httpregex.test(wpkg_base) == true) {
			...
			if (fso.FolderExists(scriptPathRelative)) {
				...
				if (fso.FolderExists(scriptPathAbsolute)) {
					...
					dinfo("Evaluated WPKG base path from absolute path: "
						+ base);
				} else {
	...
	for (var iPath = 0; iPath < xmlPaths.length; iPath++) {
		...
		if (folderLocation != null) {
			// Read data from specified XML directory (load all XML from
			// folder).
			...
			for (e.moveFirst(); !e.atEnd(); e.moveNext()) {
				...
				var extension = file.name.toString().substr(dotLocation + 1,
	...
	for (var i = 0; i < filePaths.length; i++) {
		 ...
		 * xmlns:xsl=\"http://www.w3.org/1999/XSL/Transform\" xmlns:wpkg=\"" +
		 * xmlNamespace + "\" version=\"1.0\">\r\n"; str += " <xsl:output
		 * encoding=\"utf-8\" indent=\"yes\" method=\"xml\"
		 * version=\"1.0\"/>\r\n"; str += " <xsl:template match=\"/\">\r\n"; str += " <" +
		 * "wpkg:" + rootNodeName + ">\r\n"; str += " <xsl:copy-of
		 * select=\"document('" + filePath + "')/wpkg:" + rootNodeName +
		 * "/child::*\"/>\r\n"; str += " <xsl:copy-of select=\"document('" +
		 * filePath + "')/" + rootNodeName + "/child::*\"/>\r\n"; str += " </" +
		 * "wpkg:" + rootNodeName + ">\r\n"; str += " </xsl:template>\r\n"; str += "</xsl:stylesheet>\r\n";
		 *
		 * xsl.loadXML(str);
		 */
		...
		try {
			...
			var errorMessage = "Error parsing xml '" + filePath + "': "
				+ e.description;
			if (isQuitOnError()) {
				throw new Error(errorMessage);
			} else {
		...
		if (source.parseError.errorCode != 0) {
			...
			if (rootNodeName != null && rootNodeName != rootElementName) {
				...
				var message = "Invalid XML structure found. Root element '"
					+ rootElementName
					+ "' does not match expected element name of '"
					+ rootNodeName + "'.";
				if (isQuitOnError()) {
					throw new Error(message);
				} else {
			...
			if (filePaths.length <= 1) {
				...
				var documentNodes = source.documentElement
					.selectNodes(childPath);
	...
	if (xmlType != null && xmlType == "settings" && getQueryMode() != "remote") {
		...
		if (documentElement != null) {
			var checkResultsNode = documentElement
				.selectSingleNode("checkResults");
...
function initializeConfig() {
	...
	for (var i = 0; i < param.length; i++) {
		...
		if (name == "volatileReleaseMarker") {
			volatileReleaseMarkers.push((param[i].getAttribute("value"))
				.toLowerCase());
...
function initializeLog() {
	...
	/*
	 * Set initializing flag during initialization to prevent initialization
	 * loop when logs are written during initialization.
	 */
	...
	try {
		...
		var logFileName = getLogfilePattern().replace(
			new RegExp("\\[HOSTNAME\\]", "g"), getHostname());
		 ...
		 * only. So make sure not to use the [PROFILE] placeholder if you like
		 * to remote- initialization logs (e.g. missing XML files).
		 */
		...
		newLogfilePath = new ActiveXObject("WScript.Shell")
			.ExpandEnvironmentStrings(log_file_path + "\\" + logFileName);

		// Just open the log file in case it is not opened already or append
		// mode changed from false to true.
		...
		if (logfilePath != newLogfilePath
			|| (logfileAppendMode != isLogAppend() && isLogAppend() == true)) {
			var newLogMessage = "Initializing new log file: '" + newLogfilePath
				+ "' in ";
			if (isLogAppend()) {
			...
			try {
				...
				// If new logfile path is identical to existing log file then
				// just the append mode changed.
				if (logfilePath == newLogfilePath) {
					...
					if (logfileHandler != null) {
						...
						logfileHandler.Close();
						// Replace handler.
						logfileHandler = fso.OpenTextFile(newLogfilePath,
							openMode, true, -2);
					...
					newLogfileHandler = fso.OpenTextFile(newLogfilePath,
						openMode, true, -2);
				}
				...
				newLogfilePath = new ActiveXObject("WScript.Shell")
					.ExpandEnvironmentStrings("%TEMP%\\" + logFileName);
				dinfo("Failed to open log file: " + e.description
					+ "; falling back to local logging: " + newLogfilePath);
				if (logfilePath != newLogfilePath) {
					// Open mode:
					// 2=write (use 8 for append mode)
					// true=create if not exist
					// 0=ASCII, -1=unicode, -2=system default
					newLogfileHandler = fso.OpenTextFile(newLogfilePath, 2,
						true, -2);
		...
		dinfo("Cannot initialize log file (" + err.description
			+ "), probably not all data available "
			+ "yet, stick with local log file. ");
		// Initialize local log file.
		var newLogfile = new ActiveXObject("WScript.Shell")
			.ExpandEnvironmentStrings("%TEMP%\\wpkg-logInit.log");
...
function parseArguments(argv) {
	...
	for (var i = 0; i < argv.length; i++) {
		...
		switch (argument) {
			...
			case "/synchronize":
				// Do not do anything. The /synchronize parameter is handled by
				// main() function.
				break;
 ...
 *
 * @param sort
 *            {Boolean} Set to true in order to sort package nodes.
 */
 ...
 * @return log entry in its default format:<br>
...
function info(message) {
	log(4, message);
	// PATCH SE4 : on a modifié notify pour utiliser les tooltips déployés : on informe l'user de ce que fait wpkg-se4.js
	notify(message);
	// PATCH SE4
}
...
function notify(message) {
	if (!isNoNotify()) {
		// PATCH SE4 pour utilisation du msg.exe des tooltips.
		...
		var fso = new ActiveXObject("Scripting.FileSystemObject");
		if (fso.FileExists(new ActiveXObject("WScript.Shell")
			.ExpandEnvironmentStrings(msgPath))) {
			// try msg method
			// cmd += "%COMSPEC% /U /C chcp 65001 && echo " + message + " | " +
			// msgPath + " * /TIME:" + notificationDisplayTime;
			cmd += msgPath + " * /TIME:" + notificationDisplayTime + " \""
				+ message + "\"";
		} else {
			// try net send method
			cmd += netPath + " SEND ";
		...
		// PATCH SE4 : saturation dans le cas /nonotify.
		// } else {
 ...
 *
 * @param newLocale
 *            new locale to be used starting from now.
 */
...
/**
 * Concatenates two Dictionary object and returns concatenated list. Does not
 * modify any of the dictionaries passed as paramters. Returns new Dictionary
 * object instead. If an element is listed in both dictionaries, then the value
 * of the second Dictionary is applied (overwrite). Throws error in case an
 * error occurs during dictionary append.
 *
 * @param dictionary1
 *            Dictionary to be used as a base.
 * @param dictionary2
 *            Dictionary to be appended to dictionary1.
	...
	for (var iDictionary = 0; iDictionary < dictionaries.length; iDictionary++) {
		...
		for (var iDictKey = 0; iDictKey < dictKeys.length; iDictKey++) {
			...
			try {
				...
				// dinfo("Dictionary element '" + key + "' was not defined
				// before. Creating now.");
			}
			try {
				concatenatedDictionary.Add(key, value);
			} catch (e) {
				var message = "Dictionary element '" + key + "' with value '"
					+ value + "'" + " could not be assigned to dictionary!";
				if (isQuitOnError()) {
					throw new Error(message);
				}
				error(message);
...
function exec(cmd, timeout, workdir) {
	...
	try {
		...
		while (shellExec.status == 0) {
			 ...
			 * STDERR without blocking the main WPKG program. So either you can
			 * fetch the output or wait for the program to terminate, but not
			 * both. For WPKG it's more important to handle a timeout in order
			 * to handle programs which do not terminate properly or
			 * interactively ask for input. Unfortunately sub-processes seem to
			 * be blocked if they write more than 4k of data to STDOUT and/or
			 * STDERR buffer. So make sure your commands do not print too much
			 * on the console. If in doubt you might redirect STDOUT/STDERR to a
			 * file. For example by adding "> %TEMP%\myprog-out.txt 2>&1" to the
			 * command line. See
			 * <http://www.tech-archive.net/Archive/Scripting/microsoft.public.scripting.wsh/2004-10/0204.html>
			 * for a discussion on this topic.
			 */
			// Read and discard the output buffers to prevent process blocking
			/*
			 * if (!shellExec.StdOut.AtEndOfStream) { dinfo("STDOUT: " +
			 * shellExec.StdOut.ReadAll()); } if
			 * (!shellExec.StdErr.AtEndOfStream) { dinfo("STDERR: " +
			 * shellExec.StdErr.ReadAll()); }
			 */

			for (var i = 0; i < 10 && shellExec.status == 0
				&& timeUsed < timeoutMilliseconds; i++) {
...
/**
 * Returns processor architecture as reported by Windows. Currently returns the
 * following architecture strings:
 *
 * <pre>
 * String       Description
...
function getArchitecture() {
	if (hostArchitecture == null) {
		...
		// NOTE: On 32-bit systems PROCESSOR_ARCHITECTURE is x86 even if the CPU
		// is
		// actually a 64-bit CPU
		var architecture = wshObject
			.ExpandEnvironmentStrings("%PROCESSOR_ARCHITECTURE%");
		switch (architecture) {
			case "AMD64":
	...
	if (ipAddresses == null) {
		...
		if (subKeys != null) {
			for (var i = 0; i < subKeys.length; i++) {
				...
				var service = getRegistryValue("HKLM\\" + netCards + subKeys[i]
					+ "\\ServiceName");
				if (service != null && service != "") {
					...
					if (isInterface == null) {
						dinfo("No TCP/IP Parameters for network service "
							+ service);
					} else {
						...
						if (isDHCP != null && isDHCP > 0) {
							...
							// read DHCP address
							var dhcpIP = getRegistryValue(regBase
								+ "DhcpIPAddress");
							...
							var fixedIPsRegs = getRegistryValue(regBase
								+ "IPAddress");
 ...
 * NOTE: The LCID is read from "HKCU\Control Panel\International\Locale" This is
 * the locale of the user under which WPKG is run. In case WPKG GUI is used this
 * might probably differ from the real locale of the user but at least it will
 * match the system default locale. A user working on an English installation
 * will most probably be able to understand English messages even if the users
 * locale might be set to German. I was yet unable to find any other reliable
 * way to read the locale.
 *
...
function getLocale() {
	if (LCID == null) {
		...
		if (regLocale != null) {
			...
			LCID = defaultLocale;
			dinfo("Unable to locate user locale. Using default locale: "
				+ defaultLocale);
		}
	}
 ...
 * HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Nls\Language\. This is
 * the locale under which the OS has been initially installed regardless of the
 * user locale settings.<br>
 ...
 *          http://www.microsoft.com/globaldev/reference/lcid-all.mspx for a
 *          list of possible values. Leading zeroes are stripped.
 */
function getLocaleOS() {
	if (LCIDOS == null) {
		...
		if (regLocale != null) {
			...
			dinfo("Unable to locate system locale. Using default locale: "
				+ defaultLocale);
		}
	}
...
function getRegistrySubkeys(parentKey, subLevels) {
	...
	try {
		...
		var outputParam = regProvider.ExecMethod_(enumKeyMethod.Name,
			inputParameters);
		...
		try {
			...
			/*
			 * a read error on outputParam.sNames typically means that there are
			 * no sub-keys available.
			 */
		}

	} catch (err) {
		error("Error when searching registry sub-keys at 'HKLM\\" + parentKey
...
function getRegistryValue(registryPath) {
	...
	try {
		...
		if (noSuchKeyError.replace(new RegExp(
			"HKLM\\\\SOFTWARE\\\\NOSUCHKEY\\\\"), registryPath) == readError) {
 ...
 * This allows you to call loadEnv() after operations are done to restore the
 ...
 *
 * @param packageNode
 *            The package definition to load the environment from
 */
...
/**
 * Restores environment using the given dictionary object. Variables which are
 * not defined within the dictionary object are unset. Variables which are
 * defined within the dictionary object are set to the value defined in the
 * dictionary object.
 *
 * @param environment
	...
	for (var e = new Enumerator(procEnv); !e.atEnd(); e.moveNext()) {
		...
		if (key != null && key != "") {
			if (environment.Exists(key)) {
				...
				// procEnv.Remove(key);
				// procEnv.key=environment.Item(key);
			} else {
				procEnv.Remove(key);
			}
		}
 ...
 * </pre>
 *
 * If 'ceil' is set to true then unspecified date components do not fall back to
 * "floor" (basically 0) but will be extended to the next value. This allows
 * easy comparison if the current date is within a parsed "!ceil" date and a
 * parsed "ceil" date.
 *
 * Examples:
 *
 ...
 * TIMEZONES:
 *
 * As specified by ISO 8601 the date is parsed as local date in case no time
 * zone is specified. If you define a time zone then the specified time is
 * parsed as local time for the given time zone. So if you specify "2007-11-23
 * 22:00+05:00" this will be equal to "2007-11-23 18:00+01:00" while "+01:00" is
 * known as CET as well. The special identifier "Z" is equal to "+00:00" time
 * zone offset.
 *
 ...
 *            down", see above
 * @return Date object representing the specified date. Returns null if the date
 *         cannot be parsed.
 */
function parseISODate(dateString, ceil) {
	...
	/*
	 * else if (ceilValue) { date.setFullYear(date.getFullYear() + 1); }
	 */
	// parse months
	if (matches[2]) {
	...
	if (matches[9] || timeZoneSet) {
		...
		time = Date.UTC(date.getFullYear(), date.getMonth(), date.getDate(),
		...
		switch (getRebootCmd()) {
			...
			default:
				...
				if (!fso.FileExists(getRebootCmd())) {
					...
					if (!fso.FileExists(getRebootCmd())) {
						throw new Error("Could not locate rebootCmd '"
							+ getRebootCmd() + "'.");
					}
				}
 ...
 *
 * @param array
 *            Array of dictionary objects. Each of them defining a variable
 *            (name => value).
 */
...
function setEnv(key, value) {
	...
	if (value == null) {
		dinfo("Cannot set environment variable '" + key
			+ "': No value specified!");
		return;
	}
	...
	var shell = new ActiveXObject("WScript.Shell");
	// Somehow an empty string is not accepted as string in set instruction
	// below.
	...
	// Set environment.
	procEnv.Item(key) = valueExpanded;
...
function scanUninstallKeys(nameSearched) {
	...
	if (is64bit()) {
		...
		// found)
		scanKeys
			.push("SOFTWARE\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall");
	}
	...
	for (var i = 0; i < scanKeys.length; i++) {
		...
		/*
		 * recursive registry reading is very slow with WSH. Therefore
		 * supporting Sub-keys in uninstall entries slows down uninstall key
		 * scanning dramatically. So I leave it off for the moment. Please use
		 * registry key checks if you need to check an uninstall key defined
		 * within a sub-key of the uninstall registry location
		 */
		// var keyNames = getRegistrySubkeys(regPath, -1);
		var keyNames = getRegistrySubkeys(regPath, 0);
		/*
		 * for (var k=0; k < keyNames.length; k++) { dinfo("Uninstall key: " +
		 * keyNames[k]); }
		 */
				...
				if (displayName == nameSearched) {
					...
					try {
						...
						dinfo("Unable to match uninstall key with regular expression. "
							+ "Usually this means that the string '"
							+ nameSearched
 ...
 *
 * The algorithm is supposed to deliver "human" results. It's not just comparing
 * numbers but also allows versions with characters.
 *
...
function versionCompare(a, b) {
	...
	var length = 0;
	versionA.length >= versionB.length ? length = versionA.length
		: length = versionB.length;
	var result = EQUAL;
	...
	for (var i = 0; i < length; i++) {
		...
		var versionParts = 0;
		versionPartsA.length > versionPartsB.length ? versionParts = versionPartsA.length
			: versionParts = versionPartsB.length;
		...
		for (var j = 0; j < versionParts; j++) {
			...
			dinfo("Comparing version fragments: '" + versionPartA + "' <=> '"
				+ versionPartB + "'");
			...
			var numCharLength = 0;
			numCharPartsA.length > numCharPartsB.length ? numCharLength = numCharPartsA.length
				: numCharLength = numCharPartsB.length;

...
'             - Gére les remontées des rapports
'
'
'
' Syntaxe :  cscript wpkg-client.vbs[ /noTempo][ /cpuLoad xx]
'                  /noTempo    : pour avoir des tempo réduites (utile pour un lancement en ligne de commande sans avoir é attendre...)
...
se4fs = oShell.ExpandEnvironmentStrings("%SE4FS%")
...
'
Dim LogMode        : LogMode = 2         ' Type de log. 0=Pas de log, 2=Log derniére execution, 8=Log avec conservation de l'historique
...
Dim testCPU        : testCPU = false      ' test CPU désactivé par défaut é cause des soucis de détection sur les nouveaux CPU : 06/2013
'  Par défaut, le serveur se4 est utilisé pour déployer les applications.
'  Vous pouvez cependant préférer utiliser un autre serveur (par ex. si se4 est trés chargé).
'  Pour utiliser le serveur 'NomDuServeur' é la place de 'se4', il faut :
...
'     - permettre é adminse_name d'écrire dans \\NomDuServeur\install\wpkg\rapports
'          (un accés en lecture suffit pour les autres dossiers et fichiers de ce partage)
...
'
'  De plus, pour que la gestion de la configuration des applis continue é se faire par l'interface web de se4,
...
'    Rmq. Le nom d'utilisateur ADMINSE_NAME et le mot de passe PassWWWSE4 doivent bien sér étre adapté selon votre imagination...
'
'  Au prochain démarrage des postes, le client wpkg sera mis é jour avec son nouveau paramétrage.
'
'------------------------------------------------------------------------------------
...
If fso.FileExists(oShell.ExpandEnvironmentStrings("%SystemDrive%") & "\SambaEdu\wpkg-notempo.txt") Then
   Tempo = False
End If
...
If Not fso.FolderExists(oShell.ExpandEnvironmentStrings("%SystemDrive%") & "\SambaEdu\logs") Then
   If Not fso.FolderExists(oShell.ExpandEnvironmentStrings("%SystemDrive%") & "\SambaEdu") Then
      fso.CreateFolder(oShell.ExpandEnvironmentStrings("%SystemDrive%") & "\SambaEdu")
   End If
   fso.CreateFolder(oShell.ExpandEnvironmentStrings("%SystemDrive%") & "\SambaEdu\logs")
End If
...
dim fNameLog : fNameLog = "c:\windows\rapports\" & ComputerName & ".log"
...
' Décommenter Si les paramétres de la ligne de commande sont prioritaires devant la conf ini
'ParseArguments

Dim NoWpkgTxt : NoWpkgTxt=oShell.ExpandEnvironmentStrings("%SystemDrive%") & "\SambaEdu\nowpkg.txt"
...
' ---------------- Déconnexion se4 --------------------
' commenté car on ne doit pas supprimer les lecteurs pour tous les utilisateurs connectés
'
'RemoveNetwork
print "Fin"
...
Function EtatPoste()
   ...
   If Err.Number > 0 Then
      ...
      Err.Clear
   Else
      tf.Copy "c:\windows\rapports\" & ComputerName & ".txt", True
      If Err.Number > 0 Then
...
Function RemoveNetwork()
   ...
   'For i = 0 to oDrives.Count - 1 Step 2
    ...
    '  print "RemoveNetworkDrive " & oDrives.Item(i)
      ...
      If Err.Number > 0 Then
         print "Erreur RemoveNetworkDrive " & Z
      End If
   'Next
...
Function WpkgSynchronize() 'Exécute wpkg-se4.js pour synchroniser les applications
   ...
   '   fso.CopyFile fLocal, Z & "\wpkg\rapports\" & ComputerName & ".log", true
       fso.CopyFile fLocal, "c:\windows\rapports\" & ComputerName & ".log", true
   End If
   End Function
   ...
   If RunningStatus = "true" Then
      ...
      Else
         ...
         If ageLastLog > 60 Then ' Plus de 60mn depuis le dernier lancement
            ...
            print "RunningStatus = " & RunningStatus & ". Wpkg est indiqué en exécution depuis plus de 1h. C'est sérement une erreur."
            oShell.RegDelete "HKEY_LOCAL_MACHINE\SOFTWARE\WPKG\running"
         ...
         Else
            ...
            Else
               ...
               print "RunningStatus = " & RunningStatus & ". Wpkg est déjé indiqué en cours d'exécution. FIN."
...
Function MiseAJourClient()
   ...
   If UpdateFile(Z & "\wpkg\wpkg-client.vbs", Windir & "\wpkg-client.vbs") = 1 Then
      ...
      '   End If
      'Next
      dump TimeStamp() & " Redemarrage avec le nouveau client."
...
Function SetEnvironnement()
   ...
   If logdebug Then WPKG_OPTIONS = WPKG_OPTIONS & " /log_file_path:c:\windows\rapports /logfilePattern:" & ComputerName & ".log"
   ...
   If LogMode > 0 Then
      ...
      Else
         If WScript.Interactive Then
            tf.WriteLine "-----" + TimeStamp() & " Demarrage de wpkg sur " & ComputerName & " (Mode interactif) -----"
         Else
            tf.WriteLine "-----" + TimeStamp() & " Demarrage de wpkg sur " & ComputerName & " -----"
         End If
         If MsgErrNoRun <> "" Then
...
Function MapZ
   ...
   'Do
   '   print "Map " & Z0 & " " & UNC
   '   oNet.MapNetworkDrive Z0, UNC, False
   '   If Err.Number > 0 Then
   '      MapZok = False
   ...
   '         'Essai avec une autre unité aprés 2 échecs
   '         If Z0="x:" Then Z0="w:"
   '         If Z0="y:" Then Z0="x:"
   '         If Z0="z:" Then Z0="y:"
   '      Else
   ...
   Do While (Timer < (T0 + watchDog)) And (Not FinExec)
      If Not ReadOutput() Then
         If nTest > 5 And oExec.Status = 1 Then
            '5 sec aprés la fin d'exécution, s'il n'y a plus rien a lire, on quitte
            finOK = True
            FinExec = True
            Exit Do
         End If
...
Function UpdateFile(src, dst)
   ' Met é jour le fichier dst é partir de src et retourne
...
Function TooltipInit()
    ...
    '0. Efface le fichier de log au début de l'exécution de wpkg pour éviter que celui-ci ne soit trop important é terme.
...
Function TooltipEnd()
    ' Ajoute "FIN WPKG" au fichier de log MSGlog => met fin é l'exécution de tooltip.exe dans la session de l'utilisateur logué.
    Const ForAppend = 8
    Dim f
    'Msgbox("MSGlog," & ForAppend & " , True")
    Set f = fso.OpenTextFile(MSGlog, ForAppend, True)
    f.WriteLine("FIN WPKG")
    ' on fait une pause de 2 secondes pour permettre é tooltip.exe d'avoir le temps de lire "FIN WPKG", puisqu'il lit le fichier toutes les secondes.
    Wscript.Sleep 2000
    ' on écrit une autre ligne é la fin de wpkg-msg.txt afin d'éviter le probléme "un user ouvre la session avant que wpkg ne soit lancé" qui provoquerait immédiatement l'arrét de tooltip.exe
    f.WriteLine("")
    f.Close
End Function

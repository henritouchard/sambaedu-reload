...
new #[Title('Gestion des GPOs - SE4FS')] class extends Component {
     ...
     */
    public function boot(GpoService $service, CachedGpoLookups $cache): void
    {
...
<x-organisms.page title="Gestion des GPOs" :scrollable="false"
    ...
    <x-slot:actions>
        <div class="flex gap-2 flex-wrap">
            <button type="button" class="btn btn-outline btn-sm" wire:click="exportCsv" wire:loading.attr="disabled"
                data-testid="export-csv-btn">
                <i class="fa-solid fa-file-csv"></i>
                CSV
            </button>
            <button type="button" class="btn btn-outline btn-sm" wire:click="exportJson" wire:loading.attr="disabled"
                data-testid="export-json-btn">
                <i class="fa-solid fa-file-code"></i>
                JSON
            </button>
            {{-- Story 16.14 Q2 — Bouton "Rafraîchir cache santé" (server.admin) --}}
            <button type="button" class="btn btn-outline btn-sm"
                wire:click="refreshHealthCache" wire:loading.attr="disabled"
                wire:target="refreshHealthCache"
                data-testid="refresh-health-cache-btn"
                title="Vide le cache santé GPO (links + versionNumber) et recalcule à la demande.">
                <span wire:loading.remove wire:target="refreshHealthCache">
                    <i class="fa-solid fa-heart-pulse"></i>
                    Rafraîchir cache santé
                </span>
                <span wire:loading wire:target="refreshHealthCache">
                            ...
                            <tbody>
                                @forelse ($paginatedItems as $gpo)
                                    ...
                                    <tr class="hover:bg-sky-50 cursor-pointer"
                                        onclick="if (!event.target.closest('.native-edit-cell')) window.location.href='{{ $detailUrl }}'">
                                        <td>

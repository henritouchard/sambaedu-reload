...
/**
 * Page admin/settings/agent : console de pilotage de la flotte.
 *
 * Trois surfaces sur une seule page (convention `pages/` filesystem-router,
 * Livewire SFC, `can:server.admin`) :
 *  1. Releases & rings — voir les releases publiées, la version ciblée
 *  par ring, cibler/rollback un ring (`target()`), définir la stable par
 *  défaut (`promote()`). Tout passe par `ReleaseCreationService`.
 *  2. Enrôlements en attente — surface d'approbation porte 2, étendue
 *  à l'approbation d'un poste « inconnu » par sélection de cible.
...
<x-organisms.page title="Agent — Flotte" icon="fa-solid fa-shield-halved"
    ...
    <div class="flex flex-col gap-10">
        {{-- Surface 1 : releases & rings (pilotage via ReleaseCreationService) --}}
        <section>
            <livewire:pages::admin.settings.agent._partials.releases-rings />
        </section>

        {{-- Surface 4 : catalogue d'outils — portable Rainmeter uploadé + toggle --}}
        <section>
        ...
        {{-- Surface 2 : enrôlements en attente (porte 2, + extension) --}}
        <section>
            <h2 class="text-xl font-bold mb-3 flex items-center gap-2">

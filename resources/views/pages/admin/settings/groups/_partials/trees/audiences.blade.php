{{--
    Story 62.6 — les AUDIENCES de la recette (`roles_spec`).

    Elles sont les COLONNES de la matrice. Elles ne se confondent pas avec le
    catalogue de rôles : l'ajout propose « tout le groupe » et les rôles
    ATTRIBUABLES dans ce type ; les entrées existantes s'affichent telles qu'elles
    sont écrites dans la recette, et ne sont jamais réécrites.
--}}
<x-molecules.modal.section title="Audiences" icon="fa-users text-primary" dense>

    <p class="text-xs text-base-content/70">
        Une audience désigne <strong>qui</strong> une arborescence sert. Ce qu'elle peut faire se décide
        ensuite, <strong>dossier par dossier</strong>.
    </p>

    <div class="flex flex-wrap gap-2 mt-3" data-testid="audience-list">
    ...
    <div class="flex flex-wrap items-end gap-2 mt-4">
        <div class="flex flex-col gap-1 grow min-w-64">
            ...
            <select id="pending-audience" wire:model.live="pendingAudience" class="select select-bordered w-full"
    ...
    @if ($this->undeclaredRoles !== [])
        {{-- Review 62.3 #1 — un rôle absent de la liste n'est pas un rôle qui
             n'existe pas : c'est un rôle que CE TYPE n'a pas déclaré. On le DIT,
             avec le chemin pour l'ajouter. --}}
        <p class="text-xs text-warning mt-3" data-testid="undeclared-roles-note">

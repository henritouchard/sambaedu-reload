...
<x-organisms.page title="Utilisateurs" :scrollable="false" description="Gestion des utilisateurs et des groupes" icon="fa-solid fa-users">
    ...
    {{-- Deux barres distinctes plutôt qu'une seule à contenu conditionnel : les
         onglets « Utilisateurs » et « Groupes » n'ont ni les mêmes filtres, ni la
         même méthode de reset. --}}
    @if ($activeTab === 'users')
        @php
            $hasActiveFilters = !empty($role) || !empty($status) || !empty($group) || $quotaOverflow || $passwordDefault;
        @endphp
        <x-molecules.filter-bar class="mb-4" reset="resetFilters" reset-label="Tout effacer"
            :reset-disabled="!$hasActiveFilters">
            <div class="flex-1 min-w-[240px]">
                <x-atoms.search-input model="search" placeholder="Nom, prénom ou login..."
                    wire:model.live.debounce.350ms="search" />
            </div>

            <x-slot:actions>
                {{-- Beaucoup de filtres (rôles, statuts, groupes, audit) : ils vivent
                     dans une modale dédiée, le déclencheur reste dans la barre. --}}
                <button type="button" class="btn btn-outline btn-sm"
                    wire:click="$dispatch('toggle-users-filters-modal')">
    ...
    @else
        <x-molecules.filter-bar class="mb-4">
            <div class="flex-1 min-w-[240px]">
                <x-atoms.search-input model="groupsSearch" placeholder="Nom, nom affiché ou type..." />
            </div>

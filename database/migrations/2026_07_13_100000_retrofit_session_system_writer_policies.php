...
/**
 * RETROFIT : re-route les capacités Session écrivant sous
 * `HKCU\…\Policies\*` vers l'exécutant SYSTEM (marqueur `writer` par CLÉ de
 * `spec`, contrat §7.1/§7.6) ET retire leur hint `refresh` : les deux s'excluent,
 * `refresh` n'étant émis QUE sur les items appliqués par le compagnon.
 *
 ...
 * planning le supposait — est en LECTURE SEULE pour l'utilisateur standard
 ...
 *
 * **ROLLOUT — ORDRE OPÉRATEUR IMPÉRATIF**
 ...
 * (contrairement au piège HKU) mais AUCUN correctif non plus — le
 ...
 * applicatif, décision). `up()`/`down()` DÉCODENT le `spec` existant,
 ...
 * garde `Schema::hasTable`.
 */
return new class extends Migration
{
    /**
     * Une entrée par PROJECTION re-routée : `blocked_executables` en porte
     * DEUX (bi-projection : flag `registry` + conteneur `registry_list`).
     * `refresh` = le hint que le retrofit (`2026_07_11_100000`) avait
    ...
    public function up(): void
    {
        ...
        foreach (self::RETROFIT as $entry) {
            $this->mapSpec($entry['key'], $entry['mechanism'], $now, static function (array $spec): array {
                // Marqueur PAR CLÉ : l'exécutant dépend de l'ACL du tree de
                // CETTE clé, pas de la capacité. Sur les seules clés HKCU. Les 3 specs
                ...
                // Exclusion mutuelle refresh/writer : le hint posé
                // par le retrofit est RETIRÉ (donnée cohérente — l'effet

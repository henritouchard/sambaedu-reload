...
/**
 * Mapping refnum d'un label amont → WorkstationGroup.
 *
 ...
 *
 * ⚠️ DÉCISION DE DESIGN : rattachement PAR NOM, **pas** de FK dure
 *    vers `controlhub_contract_labels`. Les labels sont réconciliés (prune +
 *  re-create) à chaque ingestion et peuvent disparaître à la rupture du
 * lien : coupler un parc durable à un label éphémère via FK serait
 *  fragile. Aligné sur le précédent `imposed_groups.label_name` (string,
 *  sans FK) et sur la résolution-par-nom (`label:<nom>`).
 *
 * ⚠️ GARDE-FOU : aucun mot « central » dans le nom de colonne / d'index / le
 *    commentaire. Vocabulaire imposé : « amont » / `controlhub`.
 * ⚠️ Colonne nullable SANS valeur par défaut métier (null = aucun label) —
...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::table('workstation_groups', function (Blueprint $table): void {
            ...
            // Index : support de la résolution-par-nom (« tous les groupes

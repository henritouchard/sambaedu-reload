...
/**
 * Ce qu'il faut pour METTRE À JOUR une extension `app` depuis
 * l'UI, et pour que la progression d'une opération survive à un rechargement de
 * page.
 *
 * Migration **strictement ADDITIVE** : et ne sont pas
 ...
 *     Sans cette colonne, impossible de désigner, parmi les
 ...
 *     déclinaison de la règle `last_error`, puis `details` d'audit :
 *     la valeur est lue par une UI, et un message d'exception Guzzle
    ...
    public function up(): void
    {
        ...
        if (! Schema::hasTable('extension_install_runs')) {
            Schema::create('extension_install_runs', function (Blueprint $table) use ($driver): void {
                ...
                // Un run peut réussir SANS avoir rien fait :
                // l'écran de l'admin était périmé et l'état demandé était déjà
                // en place (déjà installée, déjà à jour, déjà retirée).
                // L'UI exige alors un toast INFO, pas un « terminée » qui laisse

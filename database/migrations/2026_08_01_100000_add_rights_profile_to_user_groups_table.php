...
/**
 * Le groupe d'utilisateurs porte le profil de droits.
 *
 * Décisions figées :
 *
 *  - **`rights_profile_id`, PAS `role_id`.** Trois notions homonymes
 *    coexistent déjà : `user_group_user.role` (attribut d'ARÊTE member/manager/
 * owner) et `users.role` (enum legacy scolaire). Nommer la
 *    colonne `role_id` aurait créé une collision sémantique frontale.
 *  - **FK vers `roles.id`, jamais un nom.** Les profils custom sont
 *  renommables depuis `profiles/[id]` (`save()`) : un nom stocké deviendrait
 *    pendant au premier renommage.
 * - **`restrictOnDelete`** : filet DB SOUS la garde applicative d' (les
 ...
 *
 * **Aucun seed.** Cette migration est PUREMENT
